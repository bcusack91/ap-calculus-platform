...
const questions: EntranceQuizQuestion[] = [
  ...
  { id: 'pf-ent-6a', question: '$\\int \\frac{2x+3}{x^2+3x}\\,dx =$', options: ['$\\ln|x^2+3x| + C$', '$\\frac{2x+3}{2x+3} + C$', '$\\ln|x| - \\ln|x+3| + C$', '$\\ln|2x+3| + C$'], correctIndex: 0, explanation: '$\\frac{2x+3}{x(x+3)}$: let $u = x^2+3x$, $du = (2x+3)dx$, so the integral is $\\ln|x^2+3x| + C$, which also equals $\\ln|x| + \\ln|x+3| + C$. Subtracting the logs instead would differentiate to $\\frac{3}{x(x+3)}$.', partNumber: 6, partTitle: 'Problem-Solving Workshop' },
