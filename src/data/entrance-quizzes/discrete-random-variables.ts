...
const questions: EntranceQuizQuestion[] = [
  ...
  // Part 4: Variance of Random Variables
  {
    ...
    correctIndex: 2,
    explanation: 'Var(X) = Σ[(xᵢ − μ)² · P(xᵢ)], which gives the average squared deviation from the mean. SD(X) = √Var(X).',
    partNumber: 4,
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation: 'Var(aX + b) = a² · Var(X). Adding a constant b shifts the distribution but does not change its spread, so b has no effect on variance.',
    partNumber: 5,
    partTitle: 'Linear Combinations',
  },
