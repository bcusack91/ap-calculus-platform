...
const apBioEmbeddedPool: APBioPoolQuestion[] = [
  ...
  },
  {
    ...
    options: ['Hydroxyl group (—OH)', 'Carboxyl group (—COOH) only', 'Both an amino group (—$NH_{2}$) and a carboxyl group (—COOH)', 'Phosphate group (—$PO_{4}$)'],
    correctAnswer: 2,
    explanation: 'Amino acids are defined by the presence of both an amino group (—$NH_{2}$) and a carboxyl group (—COOH) attached to a central carbon, along with a variable R group.',
    domain: 'chemistry-of-life',
    topicSlug: 'carbohydrates',
    formSet: 'A',
  },
  ...
  {
    question: 'The Calvin cycle uses $CO_{2}$, ATP, and NADPH to produce:',
  ...
  },
  {
    ...
    options: ['$NAD^{+}$', 'FAD', 'Oxygen $(O_{2})$', 'Carbon dioxide $(CO_{2})$'],
    correctAnswer: 2,
    explanation: 'Oxygen is the final electron acceptor in the ETC. It combines with electrons and $H^{+}$ ions to form water. Without $O_{2}$, the chain stalls and ATP production via oxidative phosphorylation stops.',
    domain: 'cellular-energetics',
    topicSlug: 'cellular-respiration',
    formSet: 'B',
  },
  {
    ...
    options: ['Lactate', 'Ethanol and $CO_{2}$', 'Acetyl-CoA', 'Citrate'],
    correctAnswer: 1,
    explanation: 'Yeast undergoes alcoholic fermentation, converting pyruvate to ethanol and $CO_{2}$. This regenerates $NAD^{+}$ so glycolysis can continue in the absence of oxygen.',
    domain: 'cellular-energetics',
    topicSlug: 'cellular-respiration',
    formSet: 'both',
  },
  ...
  },
  {
    ...
    options: ['$G_{1}/S$ checkpoint', '$G_{2}/M$ checkpoint', 'M checkpoint (spindle assembly checkpoint)', 'S phase checkpoint'],
    correctAnswer: 2,
  ...
  },
  {
    ...
    options: [
      ...
      'atmospheric $N_{2}$ is converted to ammonia $(NH_{3})$ by certain bacteria',
      'decomposers release nitrogen from organic matter',
      'lightning converts $N_{2}$ into nitrogen oxides in the atmosphere',
    ],
    correctAnswer: 1,
    explanation: 'Nitrogen fixation is performed primarily by certain prokaryotes (e.g., Rhizobium in root nodules of legumes) that convert atmospheric $N_{2}$ into ammonia, making nitrogen available to plants.',
    domain: 'ecology',
    topicSlug: 'community-ecology',
    formSet: 'both',
  },
...
export function scoreAPBioDiagnostic(
  ...
  const examWeight = (id: string) => AP_BIO_DOMAINS.find(d => d.id === id)?.questionTarget ?? 0
