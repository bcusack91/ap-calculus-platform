// AUTO-GENERATED by scripts/write-aas-all.py — do not edit by hand.
export const aasAfricanKingdomsPart5Data = {
  topicSlug: "aas-african-kingdoms",
  sections: [
    {
      id: "aasak5-intro",
      type: 'text' as const,
      content: "# 👑 African Kingdoms & Civilizations\n\n**Part 5 of 7 — Continuity & Change Over Time**\n\n---\n\n## Tracing African Kingdoms through c. 700–1600 CE\n\nAP CCOT (Continuity and Change Over Time) prompts ask: *what changed, what stayed the same, and what drove the change?*\n\n| Period | What changed | What persisted |\n|---|---|---|\n| Early period | New institutions emerge | African cultural retentions persist |\n| Middle period | Mechanisms of trans-saharan trade expand | Centralized empires continues |\n| Late period | Outcomes shift due to external pressure | Structural features endure |\n"
    },
    {
      id: "aasak5-change",
      type: 'text' as const,
      content: "## Specific moments of change\n\n| Event | Significance |\n|---|---|\n| **1591 Battle of Tondibi** | Moroccan musketeers crush Songhai cavalry, ending the empire. |\n| **11th–15th c. Great Zimbabwe** | Stone capital of ~18,000 controlling gold–ivory routes to Sofala. |\n| **c. 800–1500 Swahili city-states** | Kilwa, Mombasa, Mogadishu integrate the East African coast into Indian Ocean trade; Swahili language emerges. |\n\n### Strong CCOT sentence\n\n> Although trans-saharan trade continued throughout c. 700–1600 CE, the events of c. 800–1500 Swahili city-states marked a decisive shift in how Black communities engaged with it."
    },
    {
      id: "aasak5-quiz",
      type: 'multiple-choice' as const,
      content: "**Concept Check** 🎯",
      exercise: {
        "questions": [
          {
            "question": "Which best describes continuity within African Kingdoms?",
            "options": [
              "Aspects such as centralized empires persisted even as other features changed.",
              "Nothing about this topic ever changed.",
              "Continuity is irrelevant to AP scoring.",
              "Only the most recent decade matters."
            ],
            "correctAnswer": 0,
            "explanation": "Continuity = elements that persist across change — a key AP analytical category."
          },
          {
            "question": "Which event marks a decisive change within African Kingdoms?",
            "options": [
              "c. 800–1500 Swahili city-states — Kilwa, Mombasa, Mogadishu integrate the East African coast into Indian Ocean trade; Swahili language emerges.",
              "An anecdote with no historical impact.",
              "A 21st-century news headline unrelated to the period.",
              "A purely literary aesthetic with no political effect."
            ],
            "correctAnswer": 0,
            "explanation": "Specific dated events anchor change-over-time arguments."
          }
        ]
      }
    },
    {
      id: "aasak5-input",
      type: 'input-boxes' as const,
      content: "**Applied Recall** ✍️\n\n1) An aspect of African Kingdoms that persisted: ___\n\n2) An event that marked decisive change: ___\n\n3) The AP analytical category for tracking these together: ___\n",
      exercise: {
        "boxes": 3,
        "correctAnswers": [
          "Centralized empires",
          "c. 800–1500 Swahili city-states",
          "continuity and change"
        ],
        "hint1": "Hint: Mali and Songhai built tax systems, standing armie…",
        "hint2": "Hint: Kilwa, Mombasa, Mogadishu integrate the East Afric…",
        "hint3": "Hint: AP CCOT",
        "explanation": "AP CCOT prompts demand both continuity and change with specific evidence."
      }
    },
    {
      id: "aasak5-dropdown",
      type: 'dropdown-select' as const,
      content: "**Continuity vs Change** 🔍",
      exercise: {
        "dropdowns": [
          {
            "label": "Continuity in this unit is best illustrated by ___",
            "options": [
              "Centralized empires",
              "c. 800–1500 Swahili city-states",
              "An unrelated modern slogan",
              "A topic from a different course"
            ]
          },
          {
            "label": "Change in this unit is best illustrated by ___",
            "options": [
              "c. 800–1500 Swahili city-states",
              "Centralized empires",
              "An anecdote with no date",
              "An unrelated personal opinion"
            ]
          },
          {
            "label": "An AP CCOT response should ___",
            "options": [
              "Identify both continuities and changes with specific evidence",
              "Pick only one without analysis",
              "Avoid using dates",
              "Replace evidence with opinion"
            ]
          }
        ],
        "correctAnswers": [
          "Centralized empires",
          "c. 800–1500 Swahili city-states",
          "Identify both continuities and changes with specific evidence"
        ],
        "hint1": "Continuity = persistence.",
        "hint2": "Change = decisive shift, named event.",
        "hint3": "CCOT requires both halves.",
        "explanation": "AP CCOT prompts always require both continuity and change with explicit evidence."
      }
    },
    {
      id: "aasak5-applied",
      type: 'multiple-choice' as const,
      content: "**AP-Style Application** 🎯",
      exercise: {
        "questions": [
          {
            "question": "Which CCOT thesis is strongest?",
            "options": [
              "Although centralized empires continued, c. 800–1500 Swahili city-states marked a decisive change in African Kingdoms.",
              "Nothing changed and nothing stayed the same.",
              "Change is impossible to identify in history.",
              "AP rubrics do not reward CCOT analysis."
            ],
            "correctAnswer": 0,
            "explanation": "Strong CCOT theses pair specific continuities with specific changes."
          },
          {
            "question": "Which evidence move is most valuable in a CCOT response?",
            "options": [
              "Naming a specific dated event AND a continuity within the same paragraph.",
              "Listing dates without analysis.",
              "Replacing evidence with a moral judgment.",
              "Citing only post-2010 news stories."
            ],
            "correctAnswer": 0,
            "explanation": "AP CCOT scoring rewards paired specificity."
          }
        ]
      }
    }
  ]
}
