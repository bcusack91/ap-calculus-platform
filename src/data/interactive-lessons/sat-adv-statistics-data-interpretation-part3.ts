...
If you find yourself averaging two averages, stop — you have almost certainly walked into the item's main trap.`
    ...
    },
    {
      ...
      correctAnswer: 1,
      explanation: 'The combined mean of 15 lies 3 above Sample A’s mean and 5 below Sample B’s mean, so the sizes are in the ratio $5 : 3$ with A the larger: Sample B has $30(3/5) = 18$ measurements. Verifying with totals, $\\frac{30(12) + 18(20)}{48} = \\frac{720}{48} = 15$. The $50$ choice reverses the lever, computing $30(5/3) = 50$, which would pull the combined mean down toward 12 rather than up to 15. The $90$ choice is the raw shortfall in total, $15(30) - 12(30) = 90$, treated as a count of measurements instead of a sum. The $48$ choice is the combined sample size, an intermediate value.'
    },
    {
      id: 'sdi-adv-p3-q3',
      type: 'quiz' as const,
      ...
      correctAnswer: 2,
      explanation: 'The recorded total was $40(72.5) = 2900$. The entry was 72 points too low, so the corrected total is $2900 + 72 = 2972$ and the corrected mean is $2972/40 = 74.3$. The $70.7$ choice subtracts the 72 instead of adding it, the sign error that comes from not asking which direction the mistake pushed the total. The $72.5$ choice assumes one bad entry among forty cannot move the mean, but a 72-point error spread over 40 scores moves it by exactly 1.8. The $144.5$ choice adds the full 72 to the mean rather than to the total, treating a single-score correction as though it applied to every score.'
    },
    {
      id: 'sdi-adv-p3-q4',
      type: 'quiz' as const,
    ...
    },
    {
      ...
      correctAnswer: 3,
      explanation: 'Write the ordered set as $13, b, 21, d, 41$ with $13 \\le b \\le 21 \\le d \\le 41$. The mean of 24 fixes the total at $5(24) = 120$, so $13 + b + 21 + d + 41 = 120$ and $b + d = 45$. Since the pair has a fixed sum, maximizing $d$ means minimizing $b$: take $b = 13$, giving $d = 32$, which satisfies $21 \\le 32 \\le 41$. The $24$ choice pushes $b$ to its maximum of 21 instead of its minimum, which minimizes $d$ rather than maximizing it. The $22.5$ choice splits 45 evenly between $b$ and $d$, and also violates the requirement that the values be integers. The $41$ choice is the largest value in the set, the fifth number, not the second-largest.'
    }
  ]
};
