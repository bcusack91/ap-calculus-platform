      ...
      exercise: {
        questions: [
          ...
          },
          {
            ...
            correctAnswer: 0,
            explanation: `SUSTAINED cortisol elevation is the signature of the **HPA axis** (hypothalamus→CRH→pituitary→ACTH→adrenal CORTEX→cortisol), and prolonged coping with ongoing immune suppression is the **resistance** stage of Selye's GAS. The alarm stage is the acute, epinephrine-driven sympathetic–adrenal-medullary burst, which is brief, not month-long. The parasympathetic pathway is "rest-and-digest," not the stress response, so that option fails on pathway alone even though immune suppression can also accompany exhaustion. Attributing HPA output to epinephrine is also wrong — the HPA's product is cortisol.`
          }
        ]
      }
    },
