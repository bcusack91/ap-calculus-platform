...
- **Focus on the argument, not the art itself** — you don't need to visualize the work being discussed`
    ...
    {
      id: 'cp6-worked-example',
      type: 'text' as const,
      content: `### Worked Example — Reading an Aesthetics Passage
...
1. **Find the claim.** The author's thesis is the reversal: photography was expected to "kill painting" but instead *liberated* it. Watch the structure — a popular prophecy stated, then overturned.
2. **Identify the value judgments.** The author treats this liberation as a *good* thing ("liberation," "declaration of independence"). Tone toward the Impressionists is approving.
3. **Track the historical logic.** This is the "movements as responses" pattern. Impressionism is framed as a *response* to photography's takeover of "mere resemblance."
4. **Don't visualize — interpret.** You do not need to picture an Impressionist canvas. You need the relationship: photography handles likeness → painting is freed to explore "color, gesture, and the structure of seeing."
5. **Predict the application question.** CARS will ask you to extend this logic to a new technology displacing an art form. The author's principle: when a new medium absorbs an art's utilitarian function, the older art is freed to pursue its distinctive aims.

**The trap:** An answer claiming the author thinks "Impressionism rejected accuracy because accuracy is worthless" overstates. The author says the Impressionists declared *independence* from accuracy, not that accuracy is bad.

> **Takeaway:** Aesthetics passages are arguments in disguise. Strip away the art vocabulary and you usually find a familiar move — a prophecy reversed, a movement explained as a reaction, a value quietly endorsed.`
    },
    {
      id: 'cp6-quiz1',
...
- Strip away the art vocabulary to find the underlying argument (a prophecy reversed, a movement as reaction, a value endorsed)
- Application questions extend the passage's principle to a new medium or movement — match the logic, not the example`
    }
  ]
};
