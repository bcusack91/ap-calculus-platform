...
const questionPool: PoolQuestion[] = [
  ...
  { question: '2/3 × 3/4 = ?', options: ['1/2', '6/7', '5/12', '8/9'], correctAnswer: 0, explanation: '(2 × 3)/(3 × 4) = 6/12 = 1/2.', domain: 'fractions-decimals', topicSlug: 'operations-with-fractions', formSet: 'B' },
...
export function scorePreAlgebraDiagnostic(form: number, questions: PreAlgebraQuestion[], answers: Record<number, number>): PreAlgebraResults {
  ...
  const domainWeight = (id: string) => PREALGEBRA_DOMAINS.find(dom => dom.id === id)?.questionTarget ?? 0
