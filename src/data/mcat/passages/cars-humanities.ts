...
The traditional response is to grade translations by their losses: this one sacrifices rhyme for sense, that one sense for rhyme. The metaphor of loss presumes an original plenitude the translation can only diminish. But readers of the original are not in possession of this plenitude either. The native reader of the sixteenth century read with assumptions no modern reader shares; the poem has been "translated" by time for its own countrymen. Its topical jokes have gone quiet; its plainest words have acquired an antique glow their first readers could not have heard; the margin of glosses in a modern edition is a translation bound alongside its original. If loss disqualifies, every reading is disqualified.
...
The objection writes itself: functional fidelity licenses anything; the translator becomes an author with an alibi. The objection would be fatal if effects could be produced at will from any material. They cannot. The translator who seeks the original's effect is constrained at every phrase by the original's structure of effects — a far more demanding master than the dictionary. The original spends three stanzas preparing the blow it strikes in the last line; a version that strikes early, whatever its local beauties, has failed at the one thing it was for. Effects have an anatomy — a preparation, a delay, a release — and that anatomy can be honored or botched in any vocabulary. The libertine translator fails not by being unfaithful but by being lazy: producing the easy effect rather than the earned one.

What follows for readers is uncomfortable. If translation is impossible in the strict sense and yet translations plainly succeed — we weep at Tolstoy without Russian — then meaning must be sturdier and more portable than the betrayal epigram assumes, and also less proprietary. The text was never a vault. It was always a score, awaiting performance. Two performances of the same score may differ in tempo, in color, in a hundred decisions of execution, and both be faithful; yet no one calls performance lawless, because a wrong note is still a wrong note, and a reading that cannot be traced to the page is not a reading of it. Translation deserves the same double verdict we grant the performer without a second thought: endless legitimate variety, under a discipline that never relaxes.`,
    questions: [
      {
...
The honest ruin, then, is a genre error. It mistakes buildings for texts and visitors for readers, when a building is nearer to a piece of music, and a restoration nearer to a performance. We do not demand that an orchestra play a damaged score with the gaps left audible. When bars have gone missing, performers complete them from the composer's sketches and the surviving parts, and the completion is judged by whether the music carries — not by whether the seam can be heard. The scholarship is not suppressed; it lives where scholarship belongs, in the critical edition and the program note, available to anyone who asks. A conductor who halted the concert to announce each emendation would not be praised for honesty; he would have changed genres, from performance to lecture. That is what the legible repair does to a nave. And where nothing survives to guide completion, the performer falls silent rather than invent — the blank volume is that silence, and it is honorable. The rest of the time, the stones owe us what the orchestra owes us, which is the work.`,
    questions: [
      {
