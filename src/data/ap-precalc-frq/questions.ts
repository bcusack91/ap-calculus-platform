...
const precalcFRQs: PrecalcFRQ[] = [
  ...
  // ===== LONG FRQ 2: Trigonometric Modeling =====
  {
    ...
    parts: [
      ...
      },
      {
        ...
        maxPoints: 1,
        rubric: [
          { points: 1, description: 'Finds t = 11 and t = 19 (11:00 AM and 7:00 PM)', keywords: ['t = 11', 't = 19', '11 am', '7 pm', 'two times'] },
        ],
        sampleAnswer: '$77 = 18\\sin\\left(\\frac{\\pi}{12}(t-9)\\right) + 68$ → $\\sin\\left(\\frac{\\pi}{12}(t-9)\\right) = \\frac{1}{2}$\n\nSolutions in one cycle: $\\frac{\\pi}{12}(t-9) = \\frac{\\pi}{6}$ → $t = 11$, and $\\frac{\\pi}{12}(t-9) = \\frac{5\\pi}{6}$ → $t = 19$.\n\n$T = 77$°F at $t = 11$ (11:00 AM) and $t = 19$ (7:00 PM).',
      },
    ],
  },
  ...
  // ===== SHORT FRQ 2: Logarithmic Equations =====
  {
    ...
    prompt: 'Consider the equation $\\log_2(x + 3) + \\log_2(x - 1) = 5$.',
    totalPoints: 4,
    timeRecommendation: '~8 minutes',
    calculatorAllowed: false,
    parts: [
      {
        ...
        sampleAnswer: '$\\log_2((x+3)(x-1)) = 5$ (product rule for logarithms).',
      },
      {
        label: 'b',
        ...
        maxPoints: 2,
        rubric: [
          { points: 1, description: 'Converts to exponential: (x+3)(x−1) = 32', keywords: ['(x+3)(x-1) = 32', '2^5 = 32', 'exponential form'] },
          { points: 1, description: 'Solves the quadratic: x = 5 or x = −7, and rejects x = −7 (both logarithm arguments must be positive), keeping x = 5', keywords: ['x = 5', 'reject', '-7', 'domain'] },
        ],
        sampleAnswer: '$(x+3)(x-1) = 2^5 = 32$\n\n$x^2 + 2x - 3 = 32$ → $x^2 + 2x - 35 = 0$ → $(x+7)(x-5) = 0$\n\n$x = 5$ or $x = -7$. Both logarithm arguments must be positive, which requires $x > 1$, so $x = -7$ is rejected.\n\n$x = 5$.',
      },
      {
        label: 'c',
  ...
  // ===== SHORT FRQ 5: Sequences & Series =====
  {
    ...
    parts: [
      ...
      {
        label: 'b',
        prompt: 'Find the 10th term of the sequence.',
        maxPoints: 1,
        rubric: [
          { points: 1, description: '$a_{10}$ = $3(2)^{9}$ = 1536', keywords: ['1536', '3(2)^9', '2^9'] },
        ],
        sampleAnswer: '$a_{10} = 3 \\cdot 2^{10-1} = 3 \\cdot 512 = 1536$.',
      },
      {
        label: 'c',
        prompt: 'Explain why the terms of this sequence can be modeled by an exponential function of $n$, and identify the growth factor of that function.',
        maxPoints: 1,
        rubric: [
          { points: 1, description: 'Successive terms change by a constant RATIO (equal multiplicative rate of change), the defining property of exponential functions; the growth factor is 2', keywords: ['constant ratio', 'multiplicative', 'exponential', 'growth factor', '2'] },
        ],
        sampleAnswer: 'Over equal steps in $n$, the terms change by the constant factor 2 — a constant multiplicative rate of change, which is exactly how exponential functions grow. The sequence fits $f(n) = 3 \\cdot 2^{n-1}$, an exponential function with growth factor 2.',
      },
    ],
  },
