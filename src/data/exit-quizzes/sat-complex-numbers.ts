...
const questionPool: QuestionTemplate[] = [
  ...
  },
  {
    ...
    generate() {
      ...
      const correct = `$${re} ${imSign} ${absIm}i$`
      const { options, correctIndex } = makeStringOptions(correct, [
        `$${a * c + b * d} + ${a * d - b * c}i$`,
        `$${re} ${im >= 0 ? '-' : '+'} ${absIm}i$`,
        `$${re + 1} + ${absIm}i$`
      ...
      return {
        ...
        question: `$(${a} + ${b}i)(${c} + ${d}i) = ?$`,
        options, correctIndex,
        explanation: `Use FOIL: $${a}\\cdot${c} + ${a}\\cdot${d}i + ${b}i\\cdot${c} + ${b}i\\cdot${d}i = ${a * c} + ${a * d}i + ${b * c}i + ${b * d}i^2 = ${a * c} + ${a * d + b * c}i - ${b * d} = ${re} ${imSign} ${absIm}i$.`
      }
    }
  },
  {
  ...
  },
  {
    ...
    generate() {
      ...
      return {
        ...
        options, correctIndex,
        explanation: `$(${a} + ${b}i)^2 = ${a * a} + 2(${a})(${b}i) + (${b}i)^2 = ${a * a} + ${2 * a * b}i - ${b * b} = ${re} + ${im}i$.`
      }
    }
  },
  ...
  },
  {
    ...
    generate() {
      // 2/(bi) rationalize
      const b = randInt(2, 6)
      const correct = `$-\\frac{2}{${b}}i$`
      const { options, correctIndex } = makeStringOptions(correct, [
        `$\\frac{2}{${b}}i$`, `$\\frac{2i}{${b}}$`, `$-\\frac{2i}{${b * b}}$`
      ])
      return {
        id: this.id, category: this.category,
        question: `Simplify $\\frac{2}{${b}i}$.`,
        options, correctIndex,
        explanation: `$\\frac{2}{${b}i} \\cdot \\frac{-i}{-i} = \\frac{-2i}{${b}i^2} = \\frac{-2i}{-${b}} = \\frac{2i}{${b}}$... Actually: $\\frac{2}{${b}i} \\cdot \\frac{i}{i} = \\frac{2i}{${b}i^2} = \\frac{2i}{-${b}} = -\\frac{2}{${b}}i$.`
      }
    }
  },
  ...
  // ===== QUADRATICS & COMPLEX ROOTS (8 questions) =====
  {
    ...
    generate() {
      ...
      const correct = `$\\pm ${root}i$`
      const { options, correctIndex } = makeStringOptions(correct, [
        `$\\pm ${root}$`, `$${root}i$`, `$\\pm ${c}i$`
      ])
      return {
        id: this.id, category: this.category,
        question: `Solve $x^2 + ${c} = 0$.`,
        options, correctIndex,
        explanation: `$x^2 = -${c}$ → $x = \\pm\\sqrt{-${c}} = \\pm ${root}i$.`
      }
    }
  },
  {
  ...
  },
  {
    ...
    category: 'Quadratics & Complex Roots',
    generate() {
      // x² + bx + c = 0 has roots that sum to -b and multiply to c
      const r1 = randInt(1, 5)
      const r2 = randInt(1, 5)
      const sum = 2 * r1 // real part sum = 2a
      const prod = r1 * r1 + r2 * r2
      const correct = `$${prod}$`
      const { options, correctIndex } = makeStringOptions(correct, [
        `$${r1 * r1 - r2 * r2}$`, `$${sum}$`, `$${2 * r1 * r2}$`
      ])
      return {
        id: this.id, category: this.category,
