...
import {
  generateAPMicroDiagnosticTest,
  scoreAPMicroDiagnostic,
  pickNextForm,
  type APMicroDiagnosticTestData,
  type APMicroDiagnosticResults,
} from '@/data/ap-micro-diagnostic'
...
export default function APMicroDiagnosticPage() {
  ...
  const [testData, setTestData] = useState<APMicroDiagnosticTestData | null>(null)
  const [results, setResults] = useState<APMicroDiagnosticResults | null>(null)
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.push('/auth/signin?callbackUrl=/ap-micro-diagnostic')
    }
  }, [status, router])

  useEffect(() => {
    if (status === 'authenticated') {
      fetch('/api/ap-micro-diagnostic/history')
  ...
  const startTest = useCallback(() => {
    ...
    const data = generateAPMicroDiagnosticTest(form)
  ...
  const handleFinish = useCallback(async () => {
    ...
    const diagnosticResults = scoreAPMicroDiagnostic(testData.form, testData.questions, answersRecord)
    setResults(diagnosticResults)
    setPhase('results')

    try {
      await fetch('/api/ap-micro-diagnostic/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          category: `ap-micro-diagnostic-${testData.form}`,
      ...
      const histRes = await fetch('/api/ap-micro-diagnostic/history')
      ...
      if (challengeToken) {
        const challengeRes = await fetch(`/api/diagnostic-challenges/${challengeToken}/submit`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            category: `ap-micro-diagnostic-${diagnosticResults.form}`,
  ...
  if (phase === 'testing' && testData) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-cyan-50 py-6 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          ...
          <div className="flex items-center justify-between">
            ...
            {currentIndex < testData.questions.length - 1 ? (
              ...
              <button onClick={handleFinish} className="rounded-xl bg-gradient-to-r from-teal-600 to-cyan-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:shadow-lg">Submit Diagnostic</button>
            )}
          </div>
        </div></div>
      </div>
  ...
  if (phase === 'results' && results) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-cyan-50 py-8 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          <h2 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">AP Microeconomics Diagnostic Results</h2>

          <div className="mb-8 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800"><p className="text-sm text-gray-500 dark:text-gray-400">Estimated AP Score</p><p className="text-5xl font-black text-teal-600 dark:text-teal-400">{results.estimatedAPScore}</p><p className="text-xs text-gray-500 dark:text-gray-400">out of 5</p></div>
          ...
          <DiagnosticChallengeCard
            category={`ap-micro-diagnostic-${results.form ?? 1}`}
          ...
          {results.recommendedTopics.length > 0 && !planAccess.loading && !planAccess.canAccess && <DiagnosticPlanPaywall />}
            ...
            <div className="mb-8 rounded-2xl border-2 border-teal-300 bg-teal-50 p-6 dark:border-teal-700 dark:bg-teal-900/20">
              ...
              <p className="mb-4 text-sm text-teal-600 dark:text-teal-400">Based on your results, we recommend reviewing these {results.recommendedTopics.length} module{results.recommendedTopics.length > 1 ? 's' : ''}.</p>
              <div className="space-y-2">
                {results.recommendedTopics.map((topic, i) => (
                  <Link key={topic.slug} href={`/topics/${topic.slug}/interactive`} className="flex items-center justify-between rounded-xl border border-teal-200 bg-white px-4 py-3 transition hover:border-teal-400 hover:shadow-sm dark:border-teal-700 dark:bg-gray-800 dark:hover:border-teal-500 group">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-teal-700 dark:group-hover:text-teal-400">{topic.name}</span>
            ...
            <StartStudyPlanButton diagnosticPath="/ap-micro-diagnostic" className="mb-8" />
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Link href="/courses/ap-microeconomics" className="flex-1 rounded-xl bg-gradient-to-r from-teal-600 to-cyan-600 py-3 text-center font-semibold text-white shadow transition hover:shadow-lg">Browse All Modules</Link>
            <Link href="/ap-micro-score-predictor" className="flex-1 rounded-xl border border-teal-300 py-3 text-center font-semibold text-teal-700 transition hover:bg-teal-50 dark:border-teal-700 dark:text-teal-300 dark:hover:bg-teal-900/20">View Score Predictor</Link>
            <Link href="/ap-micro-daily-question" className="flex-1 rounded-xl border border-teal-300 py-3 text-center font-semibold text-teal-700 transition hover:bg-teal-50 dark:border-teal-700 dark:text-teal-300 dark:hover:bg-teal-900/20">Today&apos;s Question</Link>
          </div>
        </div></div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-cyan-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-8 sm:py-12"><div className="mx-auto max-w-2xl">
        <Link href="/courses/ap-microeconomics" className="mb-4 inline-flex items-center gap-1 text-sm text-teal-600 hover:underline dark:text-teal-400">
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
          AP Microeconomics
        </Link>
        <h1 className="mb-2 text-3xl font-bold text-gray-900 sm:text-4xl dark:text-white">💹 AP Microeconomics Diagnostic Test</h1>
        <p className="mb-8 text-lg text-gray-600 dark:text-gray-400">Find your strengths and weaknesses across all AP Microeconomics units and get a personalized study plan.</p>
        ...
        {lastRecommendedTopics && lastRecommendedTopics.length > 0 && (
          <div className="mb-6 rounded-2xl border-2 border-teal-300 bg-teal-50 p-6 dark:border-teal-700 dark:bg-teal-900/20">
            ...
            <p className="mb-3 text-sm text-teal-600 dark:text-teal-400">From your last diagnostic — review these modules, then retake the test:</p>
            <div className="space-y-2">
              {lastRecommendedTopics.map((topic, i) => (
                <Link key={topic.slug} href={`/topics/${topic.slug}/interactive`} className="flex items-center justify-between rounded-xl border border-teal-200 bg-white px-4 py-3 transition hover:border-teal-400 hover:shadow-sm dark:border-teal-700 dark:bg-gray-800 group">
                  <div className="flex items-center gap-3">
                    <span className="flex h-6 w-6 items-center justify-center rounded-full bg-teal-100 text-xs font-bold text-teal-700 dark:bg-teal-900/50 dark:text-teal-300">{i + 1}</span>
                    <span className="text-sm font-medium text-gray-800 dark:text-gray-200 group-hover:text-teal-700 dark:group-hover:text-teal-400">{topic.name}</span>
        ...
        {lastResult && (
          <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="flex items-center justify-between">
              <div><p className="text-3xl font-bold text-teal-600 dark:text-teal-400">{String(lastResult.estimatedAPScore ?? '—')}/5</p><p className="text-sm text-gray-500 dark:text-gray-400">Estimated AP Score</p></div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <ul className="mb-6 space-y-2 text-sm text-gray-600 dark:text-gray-400">
            {['~18 questions spanning all 6 AP Microeconomics units', '35 minute time limit', 'Estimated AP score (1–5) with per-unit breakdown', '3-5 personalized module recommendations', '10 unique forms — each test has different questions'].map(item => (
              <li key={item} className="flex items-start gap-2">
                <svg className="mt-0.5 h-4 w-4 shrink-0 text-teal-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                {item}
              </li>
            ))}
          </ul>
          <button onClick={startTest} className="w-full rounded-xl bg-gradient-to-r from-teal-600 to-cyan-600 px-6 py-3 font-semibold text-white shadow-lg transition hover:shadow-xl">{lastResult ? 'Take Next Diagnostic' : 'Start Diagnostic Test'}</button>
          <Link href="/ap-micro-score-predictor" className="mt-3 block text-center text-sm font-medium text-teal-700 transition hover:underline dark:text-teal-300">Prefer a quick estimate? Open the Score Predictor</Link>
          <Link href="/ap-micro-daily-question" className="mt-1 block text-center text-sm font-medium text-teal-700 transition hover:underline dark:text-teal-300">Need a warm-up first? Try today&apos;s question</Link>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="space-y-3">
            {[
              { step: '1', text: 'Take the diagnostic test — questions cover all 6 AP Microeconomics units' },
