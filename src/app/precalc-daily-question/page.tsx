...
function PrecalcDailyQuestionPageInner() {
  ...
  useEffect(() => {
    fetch('/api/precalc-daily-question')
      .then(r => r.json())
      .then(data => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-violet-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-12">
        <div className="mx-auto mb-10 max-w-3xl text-center">
          <Link href="/precalc-diagnostic" className="mb-4 inline-flex items-center gap-1 text-sm text-indigo-600 hover:underline dark:text-indigo-400">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
            Pre-Calculus
          </Link>
          <h1 className="mb-3 text-3xl font-black tracking-tight text-gray-900 sm:text-4xl dark:text-white">Pre-Calculus - Question of the Day</h1>
          <p className="text-gray-600 dark:text-gray-400">A fresh Pre-Calculus question every day. Practice trigonometry, functions, sequences, and modeling.</p>
          <p className="mt-2 text-sm font-medium text-indigo-600 dark:text-indigo-400">{new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="mx-auto max-w-2xl">
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
              <div className="rounded-t-2xl bg-gradient-to-r from-indigo-500 to-violet-500 p-4 text-white">
                <span className="text-sm font-medium opacity-80">Today&apos;s Pre-Calculus Question</span>
              </div>
              <div className="p-6">
                ...
                <div className="mb-5 space-y-2">
                  {q.question.options.map((opt, i) => {
                    ...
                    } else if (i === selected) cls += 'border-indigo-500 bg-indigo-50 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300 dark:border-indigo-500'
                    else cls += 'border-gray-200 text-gray-700 hover:border-indigo-300 hover:bg-indigo-50/50 dark:border-gray-600 dark:text-gray-300 dark:hover:border-indigo-500'
                ...
                {!revealed ? (
                  <button onClick={() => setRevealed(true)} disabled={selected === null} className="w-full rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 py-2.5 font-semibold text-white shadow transition hover:shadow-lg disabled:opacity-40">Check Answer</button>
                ) : (
        ...
        <div className="mx-auto mt-10 max-w-md text-center">
          ...
          <div className="flex justify-center gap-3">
            <Link href="/precalc-diagnostic" className="rounded-lg bg-indigo-100 px-4 py-2 text-sm font-medium text-indigo-700 transition hover:bg-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-400">Diagnostic Test</Link>
            <Link href="/precalc-score-predictor" className="rounded-lg bg-indigo-100 px-4 py-2 text-sm font-medium text-indigo-700 transition hover:bg-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-400">Score Predictor</Link>
          </div>
        </div>
      </div>
    </div>
...
export default function PrecalcDailyQuestionPage() {
  return (
    <>
      <PrecalcDailyQuestionPageInner />
      <ToolPageSeoBody subjectName="Precalculus" />
    </>
  )
}
