...
function APEnviroDailyQuestionPageInner() {
  ...
  useEffect(() => {
    fetch('/api/ap-enviro-daily-question')
      .then(r => r.json())
      .then(data => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-lime-50 via-white to-green-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-12">
        <div className="mx-auto mb-10 max-w-3xl text-center">
          ...
          <h1 className="mb-3 text-3xl font-black tracking-tight text-gray-900 sm:text-4xl dark:text-white">AP Environmental Science — Question of the Day</h1>
          <p className="text-gray-600 dark:text-gray-400">A fresh question every day covering all AP Environmental Science units!</p>
          <p className="mt-2 text-sm font-medium text-lime-600 dark:text-lime-400">{new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
        </div>

        {loading ? (
          <div className="flex justify-center py-20"><div className="h-10 w-10 animate-spin rounded-full border-4 border-lime-200 border-t-lime-600" /></div>
        ) : q ? (
          <div className="mx-auto max-w-2xl">
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
              <div className="rounded-t-2xl bg-gradient-to-r from-lime-500 to-green-500 p-4 text-white">
                <span className="text-sm font-medium opacity-80">Today&apos;s AP Environmental Science Question</span>
              </div>
              <div className="p-6">
                ...
                <div className="mb-5 space-y-2">
                  {q.question.options.map((opt, i) => {
                    ...
                    } else if (i === selected) cls += 'border-lime-500 bg-lime-50 text-lime-800 dark:bg-lime-900/30 dark:text-lime-300 dark:border-lime-500'
                    else cls += 'border-gray-200 text-gray-700 hover:border-lime-300 hover:bg-lime-50/50 dark:border-gray-600 dark:text-gray-300 dark:hover:border-lime-500'
        ...
        <div className="mx-auto mt-10 max-w-md text-center">
          ...
          <div className="flex justify-center gap-3">
            <Link href="/ap-enviro-diagnostic" className="rounded-lg bg-lime-100 px-4 py-2 text-sm font-medium text-lime-700 transition hover:bg-lime-200 dark:bg-lime-900/30 dark:text-lime-400">Diagnostic Test</Link>
            <Link href="/courses/ap-environmental-science" className="rounded-lg bg-lime-100 px-4 py-2 text-sm font-medium text-lime-700 transition hover:bg-lime-200 dark:bg-lime-900/30 dark:text-lime-400">All Topics</Link>
          </div>
        </div>
      </div>
    </div>
...
export default function APEnviroDailyQuestionPage() {
  return (
    <>
      <APEnviroDailyQuestionPageInner />
