...
import {
  generatePreCalcDiagnosticTest,
  scorePreCalcDiagnostic,
  pickNextForm,
  TOTAL_FORMS,
  type PreCalcDiagnosticTestData,
  type PreCalcDiagnosticResults,
} from '@/data/precalculus-diagnostic'
...
export default function PreCalcDiagnosticPage() {
  ...
  const [testData, setTestData] = useState<PreCalcDiagnosticTestData | null>(null)
  const [results, setResults] = useState<PreCalcDiagnosticResults | null>(null)
  ...
  useEffect(() => {
    if (status === 'unauthenticated') router.push('/auth/signin?callbackUrl=/precalc-diagnostic')
  }, [status, router])

  useEffect(() => {
    if (status === 'authenticated') {
      fetch('/api/precalc-diagnostic/history')
  ...
  const startTest = useCallback(() => {
    ...
    const data = generatePreCalcDiagnosticTest(form)
  ...
  const handleFinish = useCallback(async () => {
    ...
    const diagnosticResults = scorePreCalcDiagnostic(testData.form, testData.questions, answersRecord)
    setResults(diagnosticResults); setPhase('results')
    try {
      await fetch('/api/precalc-diagnostic/submit', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ category: `precalc-diagnostic-${testData.form}`, results: { form: diagnosticResults.form, totalCorrect: diagnosticResults.totalCorrect, totalQuestions: diagnosticResults.totalQuestions, percentage: diagnosticResults.percentage, estimatedLevel: diagnosticResults.estimatedLevel, domains: diagnosticResults.domains, recommendedTopics: diagnosticResults.recommendedTopics }, weakAreas: diagnosticResults.weakAreas, strengths: diagnosticResults.strengths.join(', ') }),
      })
      const histRes = await fetch('/api/precalc-diagnostic/history')
      ...
      if (challengeToken) {
        const challengeRes = await fetch(`/api/diagnostic-challenges/${challengeToken}/submit`, {
          ...
          body: JSON.stringify({
            category: `precalc-diagnostic-${diagnosticResults.form}`,
  ...
  if (phase === 'testing' && testData) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 py-6 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          ...
          <div className="flex items-center justify-between">
            ...
            {currentIndex < testData.questions.length - 1 ? (
              ...
              <button onClick={handleFinish} className="rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:shadow-lg">Submit Diagnostic</button>
            )}
          </div>
        </div></div>
      </div>
  ...
  if (phase === 'results' && results) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 py-8 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          <h2 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Pre-Calculus Diagnostic Results</h2>
          <div className="mb-8 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800"><p className="text-sm text-gray-500 dark:text-gray-400">Level</p><p className="text-3xl font-black text-indigo-600 dark:text-indigo-400">{results.estimatedLevel}</p></div>
          ...
          <DiagnosticChallengeCard
            category={`precalc-diagnostic-${results.form ?? 1}`}
          ...
          <div className="mb-8 rounded-2xl border border-indigo-200 bg-indigo-50 p-5 dark:border-indigo-800 dark:bg-indigo-900/20">
            ...
            <ol className="space-y-2 text-sm text-indigo-700 dark:text-indigo-400 list-decimal list-inside">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-8 sm:py-12"><div className="mx-auto max-w-2xl">
        ...
        <p className="mb-8 text-lg text-gray-600 dark:text-gray-400">Identify your strengths and weaknesses across all Pre-Calculus domains and get a personalized study plan.</p>
        ...
        {lastResult && (
          <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="flex items-center justify-between">
              <div><p className="text-3xl font-bold text-indigo-600 dark:text-indigo-400">{String(lastResult.estimatedLevel ?? '—')}</p><p className="text-sm text-gray-500 dark:text-gray-400">Estimated Level</p></div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <ul className="mb-6 space-y-2 text-sm text-gray-600 dark:text-gray-400">
            {['~30 questions spanning 6 Pre-Calculus domains', '40 minute time limit', 'Proficiency level assessment with per-domain breakdown', 'Personalized topic recommendations', `${TOTAL_FORMS} unique forms with different questions each time`].map(item => (
              <li key={item} className="flex items-start gap-2"><svg className="mt-0.5 h-4 w-4 shrink-0 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>{item}</li>
            ))}
          </ul>
        ...
        <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="space-y-3">
            {[
              { step: '1', text: 'Take the diagnostic test — questions cover all 6 Pre-Calculus domains' },
