...
export async function GET() {
  try {
    ...
    if (!course) return NextResponse.json({ error: 'AP Physics C: Mechanics course not found' }, { status: 404 })
    ...
    const units = course.categories.map((cat, idx) => ({
      unitNumber: idx + 1, name: cat.name, slug: cat.slug,
    ...
    console.error('Error fetching AP Physics C: Mechanics topics:', error)
