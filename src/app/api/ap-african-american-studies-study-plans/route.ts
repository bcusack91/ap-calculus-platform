...
import { AP_AAS_PLANS } from '@/data/ap-african-american-studies-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-african-american-studies', templateId, AP_AAS_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'ap-aas-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = AP_AAS_PLANS.map(t => ({
