...
import { AP_CHEM_PLANS } from '@/data/ap-chem-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-chemistry', templateId, AP_CHEM_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'ap-chem-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = AP_CHEM_PLANS.map(t => ({
