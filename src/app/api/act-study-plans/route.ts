...
import { ACT_PLANS } from '@/data/act-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'act-prep', templateId, ACT_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'act-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = ACT_PLANS.map(t => ({
