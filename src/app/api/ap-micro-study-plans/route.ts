...
import { AP_MICRO_PLANS } from '@/data/ap-micro-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-microeconomics', templateId, AP_MICRO_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'ap-micro-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = AP_MICRO_PLANS.map(t => ({
