...
import { AP_CSP_PLANS } from '@/data/ap-csp-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-computer-science-principles', templateId, AP_CSP_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'ap-csp-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = AP_CSP_PLANS.map(t => ({
