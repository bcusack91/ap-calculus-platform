...
import { AP_US_HISTORY_PLANS } from '@/data/ap-us-history-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-us-history', templateId, AP_US_HISTORY_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'ap-us-history-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = AP_US_HISTORY_PLANS.map(t => ({
