...
import { AP_CALCBC_PLANS } from '@/data/ap-calcbc-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-calculus-bc', templateId, AP_CALCBC_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'calcbc-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = AP_CALCBC_PLANS.map(t => ({
