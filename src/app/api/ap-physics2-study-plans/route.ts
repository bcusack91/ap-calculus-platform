...
import { AP_PHYSICS2_PLANS } from '@/data/ap-physics2-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-physics-2', templateId, AP_PHYSICS2_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'ap-physics2-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = AP_PHYSICS2_PLANS.map(t => ({
