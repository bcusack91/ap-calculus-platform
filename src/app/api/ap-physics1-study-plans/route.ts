...
import { AP_PHYSICS1_PLANS } from '@/data/ap-physics1-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-physics-1', templateId, AP_PHYSICS1_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'ap-physics1-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = AP_PHYSICS1_PLANS.map(t => ({
