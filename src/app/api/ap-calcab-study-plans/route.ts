...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-calculus-ab', templateId, AP_CALCAB_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'calcab-diagnostic', resolvedTasks, start)
