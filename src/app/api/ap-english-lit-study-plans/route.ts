...
import { AP_ENG_LIT_PLANS } from '@/data/ap-english-lit-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-english-literature', templateId, AP_ENG_LIT_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'ap-english-lit-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = AP_ENG_LIT_PLANS.map(t => ({
