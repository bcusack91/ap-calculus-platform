...
import { AP_WORLD_HISTORY_PLANS } from '@/data/ap-world-history-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-world-history', templateId, AP_WORLD_HISTORY_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'ap-world-history-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = AP_WORLD_HISTORY_PLANS.map(t => ({
