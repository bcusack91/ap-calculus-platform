...
export async function GET() {
  try {
    ...
    const { avgPct, stats, trend, confidence, masteryRate } = await gatherSubjectData(
      session.user.id, 'precalc', 'ap-precalculus', 'ap-precalculus-diagnostic',
    )

    const predictedScore = mapToAPScore(avgPct, masteryRate)

    // Add current prediction as trend point
    ...
    console.error('AP Precalculus score predictor error:', error)
