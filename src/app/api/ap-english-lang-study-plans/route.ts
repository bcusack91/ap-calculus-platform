...
import { AP_ENG_LANG_PLANS } from '@/data/ap-english-lang-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-english-language', templateId, AP_ENG_LANG_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'ap-english-lang-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = AP_ENG_LANG_PLANS.map(t => ({
