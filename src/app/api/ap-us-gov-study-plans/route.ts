...
import { AP_US_GOV_PLANS } from '@/data/ap-us-gov-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-us-government', templateId, AP_US_GOV_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'ap-us-gov-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = AP_US_GOV_PLANS.map(t => ({
