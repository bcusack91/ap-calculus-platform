...
import { AP_MACRO_PLANS } from '@/data/ap-macro-study-plans'
...
export async function POST(request: Request) {
  ...
  const template = await getContentItem(CONTENT_TYPES.studyPlanTemplate, 'ap-macroeconomics', templateId, AP_MACRO_PLANS, t => t.id)
  if (!template) {
  ...
  const { tasks: adaptiveTasks } = await applyAdaptivePriority(session.user.id, 'ap-macro-diagnostic', resolvedTasks, start)
...
export async function GET() {
  const templates = AP_MACRO_PLANS.map(t => ({
