...
function GeometryDailyQuestionPageInner() {
  ...
  useEffect(() => {
    fetch('/api/geometry-daily-question')
      .then(r => r.json())
      .then(data => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-fuchsia-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-12">
        <div className="mx-auto mb-10 max-w-3xl text-center">
          <Link href="/geometry-diagnostic" className="mb-4 inline-flex items-center gap-1 text-sm text-violet-600 hover:underline dark:text-violet-400">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
            Geometry
          </Link>
          <h1 className="mb-3 text-3xl font-black tracking-tight text-gray-900 sm:text-4xl dark:text-white">Geometry — Question of the Day</h1>
          <p className="text-gray-600 dark:text-gray-400">A fresh geometry question every day. Master shapes, proofs, area, and volume!</p>
          <p className="mt-2 text-sm font-medium text-violet-600 dark:text-violet-400">{new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
        </div>

        {loading ? (
          <div className="flex justify-center py-20"><div className="h-10 w-10 animate-spin rounded-full border-4 border-violet-200 border-t-violet-600" /></div>
        ) : q ? (
          <div className="mx-auto max-w-2xl">
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
              <div className="rounded-t-2xl bg-gradient-to-r from-violet-500 to-fuchsia-500 p-4 text-white">
                <span className="text-sm font-medium opacity-80">Today&apos;s Geometry Question</span>
              </div>
              <div className="p-6">
                ...
                <div className="mb-5 space-y-2">
                  {q.question.options.map((opt, i) => {
                    ...
                    } else if (i === selected) cls += 'border-violet-500 bg-violet-50 text-violet-800 dark:bg-violet-900/30 dark:text-violet-300 dark:border-violet-500'
                    else cls += 'border-gray-200 text-gray-700 hover:border-violet-300 hover:bg-violet-50/50 dark:border-gray-600 dark:text-gray-300 dark:hover:border-violet-500'
                ...
                {!revealed ? (
                  <button onClick={() => setRevealed(true)} disabled={selected === null} className="w-full rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-600 py-2.5 font-semibold text-white shadow transition hover:shadow-lg disabled:opacity-40">Check Answer</button>
                ) : (
...
export default function GeometryDailyQuestionPage() {
  return (
    <>
      <GeometryDailyQuestionPageInner />
      <ToolPageSeoBody subjectName="Geometry" />
    </>
  )
}
