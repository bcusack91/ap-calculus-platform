...
import {
  generateGeometryDiagnosticTest,
  scoreGeometryDiagnostic,
  pickNextForm,
  TOTAL_FORMS,
  type GeometryDiagnosticTestData,
  type GeometryDiagnosticResults,
} from '@/data/geometry-diagnostic'
...
export default function GeometryDiagnosticPage() {
  ...
  const [testData, setTestData] = useState<GeometryDiagnosticTestData | null>(null)
  const [results, setResults] = useState<GeometryDiagnosticResults | null>(null)
  ...
  useEffect(() => {
    if (status === 'unauthenticated') router.push('/auth/signin?callbackUrl=/geometry-diagnostic')
  }, [status, router])

  useEffect(() => {
    if (status === 'authenticated') {
      fetch('/api/geometry-diagnostic/history')
  ...
  const startTest = useCallback(() => {
    ...
    const data = generateGeometryDiagnosticTest(form)
    ...
    try {
      ...
      if (challengeToken) {
        const challengeRes = await fetch(`/api/diagnostic-challenges/${challengeToken}/submit`, {
          ...
          body: JSON.stringify({
            category: `geometry-diagnostic-${diagnosticResults.form}`,
  ...
  if (status === 'loading') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-sky-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
  ...
  if (phase === 'testing' && testData) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-sky-50 py-6 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          <div className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="h-2 w-32 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700"><div className="h-full rounded-full bg-teal-500 transition-all" style={{ width: `${((currentIndex + 1) / testData.questions.length) * 100}%` }} /></div>
            </div>
          ...
          <div className="mb-4"><span className="rounded-full bg-teal-100 px-3 py-1 text-xs font-semibold text-teal-700 dark:bg-teal-900/30 dark:text-teal-400">{testData.domains.find(d => d.id === q.domain)?.name ?? q.domain}</span></div>
              ...
              {q.options.map((opt, i) => {
                ...
                return (
                  <button key={i} onClick={() => { if (isEliminated) return; const updated = [...answers]; updated[currentIndex] = i; setAnswers(updated) }}
                    className={`w-full rounded-xl border px-4 py-3 text-left text-sm transition ${isSelected ? 'border-teal-500 bg-teal-50 text-teal-800 dark:bg-teal-900/30 dark:text-teal-300 dark:border-teal-500' : 'border-gray-200 text-gray-700 hover:border-teal-300 hover:bg-teal-50/50 dark:border-gray-600 dark:text-gray-300 dark:hover:border-teal-500'}`}>
          ...
          <div className="flex items-center justify-between">
            ...
            {currentIndex < testData.questions.length - 1 ? (
              <button onClick={() => setCurrentIndex(prev => prev + 1)} className="rounded-xl bg-teal-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:bg-teal-700">Next →</button>
            ) : (
              <button onClick={handleFinish} className="rounded-xl bg-gradient-to-r from-teal-600 to-sky-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:shadow-lg">Submit Diagnostic</button>
            )}
          </div>
        </div></div>
      </div>
  ...
  if (phase === 'results' && results) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-sky-50 py-8 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          <h2 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Geometry Diagnostic Results</h2>
          <div className="mb-8 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800"><p className="text-sm text-gray-500 dark:text-gray-400">Level</p><p className="text-3xl font-black text-teal-600 dark:text-teal-400">{results.estimatedLevel}</p></div>
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800"><p className="text-sm text-gray-500 dark:text-gray-400">Correct</p><p className="text-4xl font-black text-sky-600 dark:text-sky-400">{results.totalCorrect}/{results.totalQuestions}</p><p className="text-xs text-gray-500 dark:text-gray-400">{results.percentage}%</p></div>
          ...
          <DiagnosticChallengeCard
            category={`geometry-diagnostic-${results.form ?? 1}`}
          ...
          {results.recommendedTopics.length > 0 && !planAccess.loading && !planAccess.canAccess && <DiagnosticPlanPaywall />}
            ...
            <div className="mb-8 rounded-2xl border-2 border-teal-300 bg-teal-50 p-6 dark:border-teal-700 dark:bg-teal-900/20">
              <h3 className="mb-1 text-lg font-bold text-teal-800 dark:text-teal-300">🎯 Your Personalized Study Plan</h3>
              <p className="mb-4 text-sm text-teal-600 dark:text-teal-400">Based on your results, review these {results.recommendedTopics.length} topic{results.recommendedTopics.length > 1 ? 's' : ''}.</p>
              <div className="space-y-2">
                {results.recommendedTopics.map((topic, i) => (
                  <Link key={topic.slug} href={`/topics/${topic.slug}/interactive`} className="flex items-center justify-between rounded-xl border border-teal-200 bg-white px-4 py-3 transition hover:border-teal-400 hover:shadow-sm dark:border-teal-700 dark:bg-gray-800 dark:hover:border-teal-500 group">
                    <div className="flex items-center gap-3">
                      <span className="flex h-7 w-7 items-center justify-center rounded-full bg-teal-100 text-xs font-bold text-teal-700 dark:bg-teal-900/50 dark:text-teal-300">{i + 1}</span>
                      <div><span className="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-teal-700 dark:group-hover:text-teal-400">{topic.name}</span><span className={`ml-2 text-xs px-2 py-0.5 rounded-full ${topic.priority === 'high' ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' : 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400'}`}>{topic.priority === 'high' ? 'High Priority' : 'Medium Priority'}</span></div>
                    </div>
            ...
            <StartStudyPlanButton diagnosticPath="/geometry-diagnostic" className="mb-8" />
          <div className="mb-8 rounded-2xl border border-teal-200 bg-teal-50 p-5 dark:border-teal-800 dark:bg-teal-900/20">
            <h4 className="mb-2 font-semibold text-teal-800 dark:text-teal-300">🔄 How This Cycle Works</h4>
            <ol className="space-y-2 text-sm text-teal-700 dark:text-teal-400 list-decimal list-inside">
            ...
            {completedModules > 0 && <p className="mt-3 text-xs text-teal-500 dark:text-teal-400">You&apos;ve taken {completedModules} diagnostic test{completedModules > 1 ? 's' : ''} so far — keep going!</p>}
          </div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <button onClick={() => { setResults(null); setTestData(null); startTest() }} className="flex-1 rounded-xl border-2 border-teal-500 py-3 font-semibold text-teal-600 transition hover:bg-teal-50 dark:text-teal-400 dark:hover:bg-teal-900/20">Take Next Diagnostic</button>
            <Link href="/courses/geometry" className="flex-1 rounded-xl bg-gradient-to-r from-teal-600 to-sky-600 py-3 text-center font-semibold text-white shadow transition hover:shadow-lg">Browse Geometry Topics</Link>
            <Link href="/geometry-score-predictor" className="flex-1 rounded-xl border border-teal-300 py-3 text-center font-semibold text-teal-700 transition hover:bg-teal-50 dark:border-teal-700 dark:text-teal-300 dark:hover:bg-teal-900/20">View Score Predictor</Link>
            <Link href="/geometry-daily-question" className="flex-1 rounded-xl border border-teal-300 py-3 text-center font-semibold text-teal-700 transition hover:bg-teal-50 dark:border-teal-700 dark:text-teal-300 dark:hover:bg-teal-900/20">Today&apos;s Question</Link>
          </div>
        </div></div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-sky-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-8 sm:py-12"><div className="mx-auto max-w-2xl">
        <Link href="/courses/geometry" className="mb-4 inline-flex items-center gap-1 text-sm text-teal-600 hover:underline dark:text-teal-400">
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>Geometry
        </Link>
        <h1 className="mb-2 text-3xl font-bold text-gray-900 sm:text-4xl dark:text-white">📐 Geometry Diagnostic Test</h1>
        <p className="mb-8 text-lg text-gray-600 dark:text-gray-400">Identify your strengths and weaknesses across all Geometry domains and get a personalized study plan.</p>

        {lastRecommendedTopics && lastRecommendedTopics.length > 0 && (
          <div className="mb-6 rounded-2xl border-2 border-teal-300 bg-teal-50 p-6 dark:border-teal-700 dark:bg-teal-900/20">
            <h3 className="mb-1 text-base font-bold text-teal-800 dark:text-teal-300">🎯 Your Current Study Plan</h3>
            <p className="mb-3 text-sm text-teal-600 dark:text-teal-400">From your last diagnostic — review these topics, then retake the test:</p>
            <div className="space-y-2">
              {lastRecommendedTopics.map((topic, i) => (
                <Link key={topic.slug} href={`/topics/${topic.slug}/interactive`} className="flex items-center justify-between rounded-xl border border-teal-200 bg-white px-4 py-3 transition hover:border-teal-400 hover:shadow-sm dark:border-teal-700 dark:bg-gray-800 group">
                  <div className="flex items-center gap-3"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-teal-100 text-xs font-bold text-teal-700 dark:bg-teal-900/50 dark:text-teal-300">{i + 1}</span><span className="text-sm font-medium text-gray-800 dark:text-gray-200 group-hover:text-teal-700 dark:group-hover:text-teal-400">{topic.name}</span><span className={`text-xs px-2 py-0.5 rounded-full ${topic.priority === 'high' ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' : 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400'}`}>{topic.priority === 'high' ? 'High' : 'Medium'}</span></div>
                  <span className="text-teal-500 group-hover:translate-x-1 transition-transform">→</span>
                </Link>
              ))}
            </div>
        ...
        {lastResult && (
          <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="flex items-center justify-between">
              <div><p className="text-3xl font-bold text-teal-600 dark:text-teal-400">{String(lastResult.estimatedLevel ?? '—')}</p><p className="text-sm text-gray-500 dark:text-gray-400">Estimated Level</p></div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <ul className="mb-6 space-y-2 text-sm text-gray-600 dark:text-gray-400">
            {['~30 questions spanning 6 Geometry domains', '35 minute time limit', 'Proficiency level assessment with per-domain breakdown', 'Personalized topic recommendations', `${TOTAL_FORMS} unique forms with different questions each time`].map(item => (
              <li key={item} className="flex items-start gap-2"><svg className="mt-0.5 h-4 w-4 shrink-0 text-teal-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>{item}</li>
            ))}
          </ul>
          <button onClick={startTest} className="w-full rounded-xl bg-gradient-to-r from-teal-600 to-sky-600 px-6 py-3 font-semibold text-white shadow-lg transition hover:shadow-xl">{lastResult ? 'Take Next Diagnostic' : 'Start Diagnostic Test'}</button>
          <Link href="/geometry-score-predictor" className="mt-3 block text-center text-sm font-medium text-teal-700 transition hover:underline dark:text-teal-300">Prefer a quick estimate? Open the Geometry Score Predictor</Link>
          <Link href="/geometry-daily-question" className="mt-1 block text-center text-sm font-medium text-teal-700 transition hover:underline dark:text-teal-300">Need a warm-up first? Try today&apos;s Geometry question</Link>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="space-y-3">
            {[
              { step: '1', text: 'Take the diagnostic test — questions cover all 6 Geometry domains' },
              ...
              <div key={item.step} className="flex items-start gap-3">
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-teal-100 text-xs font-bold text-teal-700 dark:bg-teal-900/50 dark:text-teal-300">{item.step}</span>
