...
import {
  generateAPStatsDiagnosticTest,
  scoreAPStatsDiagnostic,
  pickNextForm,
  type APStatsDiagnosticTestData,
  type APStatsDiagnosticResults,
} from '@/data/ap-statistics-diagnostic'
...
export default function APStatsDiagnosticPage() {
  ...
  const [testData, setTestData] = useState<APStatsDiagnosticTestData | null>(null)
  const [results, setResults] = useState<APStatsDiagnosticResults | null>(null)
  ...
  useEffect(() => {
    if (status === 'authenticated') {
      fetch('/api/ap-stats-diagnostic/history')
  ...
  const handleFinish = useCallback(async () => {
    ...
    const diagnosticResults = scoreAPStatsDiagnostic(testData.form, testData.questions, answersRecord)
    setResults(diagnosticResults)
    setPhase('results')

    try {
      await fetch('/api/ap-stats-diagnostic/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          category: `ap-stats-diagnostic-${testData.form}`,
          results: {
      ...
      const histRes = await fetch('/api/ap-stats-diagnostic/history')
      if (histRes.ok) {
      ...
      if (challengeToken) {
        const challengeRes = await fetch(`/api/diagnostic-challenges/${challengeToken}/submit`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            category: `ap-stats-diagnostic-${diagnosticResults.form}`,
      ...
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50 py-6 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container">
          <div className="mx-auto max-w-3xl">
            <div className="mb-6 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="h-2 w-32 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
                  <div className="h-full rounded-full bg-indigo-500 transition-all" style={{ width: `${((currentIndex + 1) / testData.questions.length) * 100}%` }} />
                </div>
              </div>
            ...
            <div className="mb-4">
              <button onClick={() => setShowReference(true)} className="rounded-lg border border-indigo-300 px-3 py-1 text-xs font-medium text-indigo-700 transition hover:bg-indigo-50 dark:border-indigo-600 dark:text-indigo-400 dark:hover:bg-indigo-900/20">📋 Reference Sheet</button>
            ...
            <ReferenceSheetModal open={showReference} onClose={() => setShowReference(false)} courseSlug="ap-statistics" />
          </div>
        </div>
      </div>
    )
  ...
  if (phase === 'results' && results) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50 py-8 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container">
          <div className="mx-auto max-w-3xl">
            <h2 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">AP Statistics Diagnostic Results</h2>
            ...
            <div className="mb-8 grid gap-4 sm:grid-cols-3">
              <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800">
                ...
                <p className="text-5xl font-black text-indigo-600 dark:text-indigo-400">{results.estimatedAPScore}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">out of 5</p>
              </div>
              <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800">
                <p className="text-sm text-gray-500 dark:text-gray-400">Correct</p>
                <p className="text-4xl font-black text-blue-600 dark:text-blue-400">{results.totalCorrect}/{results.totalQuestions}</p>
          ...
          <DiagnosticChallengeCard
            category={`ap-stats-diagnostic-${results.form ?? 1}`}
          ...
          {testData && (
            ...
            {results.recommendedTopics.length > 0 && planAccess.canAccess && (
              <div className="mb-8 rounded-2xl border-2 border-indigo-300 bg-indigo-50 p-6 dark:border-indigo-700 dark:bg-indigo-900/20">
                ...
                <p className="mb-4 text-sm text-indigo-600 dark:text-indigo-400">
                  Based on your results, we recommend reviewing these {results.recommendedTopics.length} module{results.recommendedTopics.length > 1 ? 's' : ''}.
                </p>
                <div className="space-y-2">
            ...
            <StartStudyPlanButton diagnosticPath="/ap-stats-diagnostic" className="mb-8" />
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-8 sm:py-12">
        <div className="mx-auto max-w-2xl">
          <Link href="/courses/ap-statistics" className="mb-4 inline-flex items-center gap-1 text-sm text-indigo-600 hover:underline dark:text-indigo-400">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
            AP Statistics
          </Link>
          <h1 className="mb-2 text-3xl font-bold text-gray-900 sm:text-4xl dark:text-white">📊 AP Statistics Diagnostic Test</h1>
          <p className="mb-8 text-lg text-gray-600 dark:text-gray-400">
            Identify your strengths and weaknesses across all AP Statistics content areas and get a personalized study plan.
          </p>
          ...
          {lastResult && (
            <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-3xl font-bold text-indigo-600 dark:text-indigo-400">{String(lastResult.estimatedAPScore ?? '—')}/5</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Estimated AP Score</p>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-400">Form {String(lastResult.form ?? '—')} · {new Date(history[0].createdAt).toLocaleDateString()}<br /><Link href={`/diagnostic-review/${history[0].id}`} className="mt-1 inline-block text-xs font-semibold text-purple-600 hover:underline dark:text-purple-400" onClick={(e) => e.stopPropagation()}>Review past attempt →</Link></p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="space-y-3">
              {[
                { step: '1', text: 'Take the diagnostic test — questions cover all 4 AP Statistics content areas' },
                ...
                { step: '5', text: 'Repeat until you\'re scoring 4+ across all areas!' },
              ].map(item => (
