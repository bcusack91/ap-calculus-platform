...
export default function ApUsHistoryCompetitivePage() {
  ...
  const checkQueue = useCallback(async () => {
    try {
      ...
      else if (data.status === 'matched') { setInQueue(false); router.push(`/competitive/match/${data.matchId}?from=ap-us-history`) }
  ...
  const joinQueue = async () => {
    try {
      const res = await fetch('/api/competitive/queue', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ topicSlug: selectedTopic || 'ap-us-history', gameMode: selectedMode }) })
      const data: QueueStatus = await res.json()
      if (data.status === 'matched') router.push(`/competitive/match/${data.matchId}?from=ap-us-history`)
  ...
  const startAIPractice = async (difficulty: 'easy' | 'medium' | 'hard') => {
    try {
      ...
      if (data.matchId) router.push(`/competitive/match/${data.matchId}?from=ap-us-history`)
