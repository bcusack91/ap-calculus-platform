...
export default function ApUsHistoryCompetitivePage() {
  ...
  useEffect(() => {
    ...
    async function fetchTopics() {
      try {
        const res = await fetch('/api/competitive/ap-us-history-topics')
        if (!res.ok) throw new Error('Failed')
        setTopicsData(await res.json())
      } catch (err) { console.error('Error loading AP US History topics:', err) }
      finally { setLoading(false) }
    }
    void fetchTopics()
  }, [session])

  useEffect(() => { if (status === 'unauthenticated') router.push('/auth/signin?callbackUrl=/competitive/ap-us-history') }, [status, router])
  ...
  const checkQueue = useCallback(async () => {
    try {
      ...
      else if (data.status === 'matched') { setInQueue(false); router.push(`/competitive/${selectedMode === 'TEAM_BATTLE' ? 'team-match' : 'match'}/${data.matchId}?from=ap-us-history`) }
  ...
  const joinQueue = async () => {
    try {
      const res = await fetch(selectedMode === 'TEAM_BATTLE' ? '/api/competitive/team-queue' : '/api/competitive/queue', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ topicSlug: selectedTopic || 'ap-us-history', gameMode: selectedMode }) })
      const data: QueueStatus = await res.json()
      if (data.status === 'matched') router.push(`/competitive/${selectedMode === 'TEAM_BATTLE' ? 'team-match' : 'match'}/${data.matchId}?from=ap-us-history`)
  ...
  const startAIPractice = async (difficulty: 'easy' | 'medium' | 'hard') => {
    try {
      const res = await fetch('/api/competitive/practice-ai', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ topicSlug: selectedTopic || 'ap-us-history', gameMode: selectedMode, aiDifficulty: difficulty }) })
      const data = await res.json()
      if (data.matchId) router.push(`/competitive/${selectedMode === 'TEAM_BATTLE' ? 'team-match' : 'match'}/${data.matchId}?from=ap-us-history`)
    } catch (err) { console.error('Error:', err) }
  }

  if (status === 'loading' || loading) return (<div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 via-white to-violet-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900"><div className="animate-pulse text-xl text-gray-500">Loading AP US History Competitive Mode…</div></div>)
  if (!session) return null
  if (!topicsData) return (<div className="min-h-screen flex items-center justify-center px-4"><div className="text-center"><p className="text-xl text-gray-600 dark:text-gray-400 mb-4">Could not load AP US History topics.</p><button onClick={() => router.push('/ap-us-history')} className="text-indigo-600 hover:underline">← Back to AP US History</button></div></div>)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-violet-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="max-w-5xl mx-auto px-4 py-8">
        <button onClick={() => router.push('/ap-us-history')} className="inline-flex items-center gap-1 text-sm text-indigo-600 dark:text-indigo-400 hover:underline mb-6">← Back to AP US History</button>
        <div className="text-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-gray-900 dark:text-white mb-2">AP US History{' '}<span className="bg-gradient-to-r from-indigo-500 to-violet-500 bg-clip-text text-transparent">Competitive Mode</span></h1>
          ...
          <div className="mt-4 max-w-md mx-auto">
            ...
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5"><div className="bg-gradient-to-r from-indigo-500 to-violet-500 h-2.5 rounded-full transition-all" style={{ width: `${(topicsData.completedCount / topicsData.totalTopics) * 100}%` }} /></div>
          </div>
        </div>
                  ...
                  {unit.topics.map(topic => {
                    ...
                    return (
                      <button key={topic.slug} disabled={!isCompleted} onClick={() => setSelectedTopic(isSelected ? null : topic.slug)} className={`relative text-left p-4 rounded-xl border transition-all ${isSelected ? 'ring-2 ring-orange-500 border-orange-300 dark:border-orange-600 bg-white dark:bg-gray-800 shadow-lg' : isCompleted ? 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:shadow-md cursor-pointer' : 'border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800/50 opacity-50 cursor-not-allowed'}`}>
                        ...
                        {isCompleted && (<div className="mt-1.5 flex items-center gap-2"><div className="flex-1 bg-gray-200 dark:bg-gray-700 rounded-full h-1.5"><div className="bg-indigo-500 h-1.5 rounded-full" style={{ width: `${Math.round(topic.masteryLevel * 100)}%` }} /></div><span className="text-xs text-gray-500">{Math.round(topic.masteryLevel * 100)}%</span></div>)}
        ...
        {selectedTopic && anyCompleted && (
          <>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 text-center">
              {!inQueue ? (
                <>
                  <button onClick={joinQueue} className="px-10 py-4 bg-gradient-to-r from-indigo-500 to-violet-500 hover:from-indigo-600 hover:to-violet-600 text-white font-bold rounded-xl text-xl transition-all shadow-lg hover:shadow-xl">Find Match</button>
        ...
        {!anyCompleted && (
          <div className="text-center bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 sm:p-8">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">Finish any AP US History interactive lesson with 80%+ mastery to start competing.</p>
            <button onClick={() => router.push('/ap-us-history')} className="px-8 py-3 bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-semibold rounded-xl hover:shadow-lg transition-all">Browse AP US History Lessons</button>
          </div>
        )}
      </div>
    </div>
