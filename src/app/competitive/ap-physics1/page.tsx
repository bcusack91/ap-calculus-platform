...
export default function APPhysics1CompetitivePage() {
  ...
  useEffect(() => {
    ...
    async function fetchTopics() {
      try {
        const res = await fetch('/api/competitive/ap-physics1-topics')
        if (!res.ok) throw new Error('Failed to fetch topics')
        const data: TopicsResponse = await res.json()
        setTopicsData(data)
      } catch (err) {
        console.error('Error loading AP Physics 1 topics:', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.push('/auth/signin?callbackUrl=/competitive/ap-physics1')
    }
  }, [status, router])
  ...
  if (status === 'loading' || loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-indigo-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="animate-pulse text-xl text-gray-500">Loading AP Physics 1 Competitive Mode…</div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center">
          <p className="text-xl text-gray-600 dark:text-gray-400 mb-4">Could not load AP Physics 1 topics.</p>
          <button onClick={() => router.push('/ap-physics-1')} className="text-blue-600 hover:underline">← Back to AP Physics 1</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="max-w-5xl mx-auto px-4 py-8">
        <button onClick={() => router.push('/ap-physics-1')} className="inline-flex items-center gap-1 text-sm text-blue-600 dark:text-blue-400 hover:underline mb-6">← Back to AP Physics 1</button>

        <div className="text-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-gray-900 dark:text-white mb-2">
            AP Physics 1{' '}
            <span className="bg-gradient-to-r from-blue-500 to-indigo-500 bg-clip-text text-transparent">Competitive Mode</span>
          </h1>
          ...
          <div className="mt-4 max-w-md mx-auto">
            ...
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
              <div className="bg-gradient-to-r from-blue-500 to-indigo-500 h-2.5 rounded-full transition-all" style={{ width: `${(topicsData.completedCount / topicsData.totalTopics) * 100}%` }} />
            </div>
          </div>
        </div>
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                {[{ key: 'SPEED_RACE', icon: '⚡', title: 'Speed Race', desc: 'First to 10 correct wins!' }, { key: 'ACCURACY_CHALLENGE', icon: '🎯', title: 'Accuracy', desc: 'Highest accuracy on 20 wins!' }, { key: 'TEAM_BATTLE', icon: '👥', title: 'Team 2v2', desc: 'First team to 15 wins!' }].map(mode => (
                  <button key={mode.key} onClick={() => setSelectedMode(mode.key)}
                    className={`p-5 rounded-xl border transition-all text-left ${selectedMode === mode.key ? 'ring-2 ring-orange-500 border-orange-300 dark:border-orange-600 bg-white dark:bg-gray-800 shadow-lg' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:shadow-md'}`}>
              ...
              {!inQueue ? (
                <>
                  ...
                  <div className="mt-5 pt-5 border-t border-gray-200 dark:border-gray-700">
                    ...
                    {showAIOptions && (<div className="mt-4 flex justify-center gap-3">
        ...
        {!anyCompleted && (
          <div className="text-center bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-8">
            ...
            <button onClick={() => router.push('/courses/ap-physics-1')} className="px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold rounded-xl hover:shadow-lg transition-all">Browse AP Physics 1 Lessons</button>
          </div>
        )}
      </div>
    </div>
