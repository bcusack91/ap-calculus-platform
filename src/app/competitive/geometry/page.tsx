...
export default function GeometryCompetitivePage() {
  ...
  const checkQueue = useCallback(async () => {
    try {
      ...
      else if (data.status === 'matched') { setInQueue(false); router.push(`/competitive/match/${data.matchId}?from=geometry`) }
  ...
  const joinQueue = async () => {
    try {
      const res = await fetch('/api/competitive/queue', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ topicSlug: selectedTopic || 'geometry', gameMode: selectedMode }) })
      const data: QueueStatus = await res.json()
      if (data.status === 'matched') router.push(`/competitive/match/${data.matchId}?from=geometry`)
  ...
  const startAIPractice = async (difficulty: 'easy' | 'medium' | 'hard') => {
    try {
      ...
      if (data.matchId) router.push(`/competitive/match/${data.matchId}?from=geometry`)
