...
export default function APCalculusBCCompetitivePage() {
  ...
  useEffect(() => {
    ...
    async function fetchTopics() {
      try {
        const res = await fetch('/api/competitive/calculus-bc-topics')
        if (!res.ok) throw new Error('Failed to fetch topics')
        const data: TopicsResponse = await res.json()
        setTopicsData(data)
      } catch (err) { console.error('Error loading AP Calculus BC topics:', err) } finally { setLoading(false) }
    }
    void fetchTopics()
  }, [session])

  useEffect(() => { if (status === 'unauthenticated') router.push('/auth/signin?callbackUrl=/competitive/ap-calculus-bc') }, [status, router])
  ...
  if (status === 'loading' || loading) return (<div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-rose-50 via-white to-pink-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900"><div className="animate-pulse text-xl text-gray-500">Loading AP Calculus BC Competitive Mode…</div></div>)
  if (!session) return null
  if (!topicsData) return (<div className="min-h-screen flex items-center justify-center px-4"><div className="text-center"><p className="text-xl text-gray-600 dark:text-gray-400 mb-4">Could not load AP Calculus BC topics.</p><button onClick={() => router.push('/ap-calculus-bc')} className="text-rose-600 hover:underline">← Back to AP Calculus BC</button></div></div>)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-pink-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="max-w-5xl mx-auto px-4 py-8">
        <button onClick={() => router.push('/ap-calculus-bc')} className="inline-flex items-center gap-1 text-sm text-rose-600 dark:text-rose-400 hover:underline mb-6">← Back to AP Calculus BC</button>
        <div className="text-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-gray-900 dark:text-white mb-2">AP Calculus BC{' '}<span className="bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent">Competitive Mode</span></h1>
        ...
        {!anyCompleted && (
          <div className="text-center bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-8">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">Finish any AP Calculus BC interactive lesson with 80%+ mastery to start competing.</p>
            <button onClick={() => router.push('/courses/ap-calculus-bc')} className="px-8 py-3 bg-gradient-to-r from-rose-600 to-pink-600 text-white font-semibold rounded-xl hover:shadow-lg transition-all">Browse AP Calculus BC Lessons</button>
          </div>
        )}
      </div>
    </div>
