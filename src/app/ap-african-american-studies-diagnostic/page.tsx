...
import {
  generateAPAfricanAmericanStudiesDiagnosticTest,
  scoreAPAfricanAmericanStudiesDiagnostic,
  pickNextForm,
  type APAfricanAmericanStudiesDiagnosticTestData,
  type APAfricanAmericanStudiesDiagnosticResults,
} from '@/data/ap-african-american-studies-diagnostic'
...
export default function APAfricanAmericanStudiesDiagnosticPage() {
  ...
  const [testData, setTestData] = useState<APAfricanAmericanStudiesDiagnosticTestData | null>(null)
  const [results, setResults] = useState<APAfricanAmericanStudiesDiagnosticResults | null>(null)
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.push('/auth/signin?callbackUrl=/ap-african-american-studies-diagnostic')
    }
  }, [status, router])

  useEffect(() => {
    if (status === 'authenticated') {
      fetch('/api/ap-african-american-studies-diagnostic/history')
  ...
  const startTest = useCallback(() => {
    ...
    const data = generateAPAfricanAmericanStudiesDiagnosticTest(form)
  ...
  const handleFinish = useCallback(async () => {
    ...
    const diagnosticResults = scoreAPAfricanAmericanStudiesDiagnostic(testData.form, testData.questions, answersRecord)
    setResults(diagnosticResults)
    setPhase('results')

    try {
      await fetch('/api/ap-african-american-studies-diagnostic/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          category: `ap-aas-diagnostic-${testData.form}`,
      ...
      const histRes = await fetch('/api/ap-african-american-studies-diagnostic/history')
      ...
      if (challengeToken) {
        const challengeRes = await fetch(`/api/diagnostic-challenges/${challengeToken}/submit`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            category: `ap-aas-diagnostic-${diagnosticResults.form}`,
        ...
        <div className="container"><div className="mx-auto max-w-3xl">
          <div className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="h-2 w-32 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700"><div className="h-full rounded-full bg-purple-500 transition-all" style={{ width: `${((currentIndex + 1) / testData.questions.length) * 100}%` }} /></div>
            </div>
          ...
          <div className="mb-4"><span className="rounded-full bg-purple-100 px-3 py-1 text-xs font-semibold text-purple-700 dark:bg-purple-900/30 dark:text-purple-400">{testData.domains.find(d => d.id === q.domain)?.name ?? q.domain}</span></div>
          ...
          <div className="flex items-center justify-between">
            ...
            {currentIndex < testData.questions.length - 1 ? (
              <button onClick={() => setCurrentIndex(prev => prev + 1)} className="rounded-xl bg-purple-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:bg-purple-700">Next →</button>
            ) : (
  ...
  if (phase === 'results' && results) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-violet-50 py-8 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          <h2 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">AP African American Studies Diagnostic Results</h2>

          <div className="mb-8 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800"><p className="text-sm text-gray-500 dark:text-gray-400">Estimated AP Score</p><p className="text-5xl font-black text-purple-600 dark:text-purple-400">{results.estimatedAPScore}</p><p className="text-xs text-gray-500 dark:text-gray-400">out of 5</p></div>
          ...
          <DiagnosticChallengeCard
            category={`ap-aas-diagnostic-${results.form ?? 1}`}
          ...
          {results.recommendedTopics.length > 0 && !planAccess.loading && !planAccess.canAccess && <DiagnosticPlanPaywall />}
            ...
            <div className="mb-8 rounded-2xl border-2 border-purple-300 bg-purple-50 p-6 dark:border-purple-700 dark:bg-purple-900/20">
              ...
              <p className="mb-4 text-sm text-purple-600 dark:text-purple-400">Based on your results, we recommend reviewing these {results.recommendedTopics.length} module{results.recommendedTopics.length > 1 ? 's' : ''}.</p>
              <div className="space-y-2">
            ...
            <StartStudyPlanButton diagnosticPath="/ap-african-american-studies-diagnostic" className="mb-8" />
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Link href="/courses/ap-african-american-studies" className="flex-1 rounded-xl bg-gradient-to-r from-purple-600 to-violet-600 py-3 text-center font-semibold text-white shadow transition hover:shadow-lg">Browse All Modules</Link>
            <Link href="/ap-african-american-studies-score-predictor" className="flex-1 rounded-xl border border-purple-300 py-3 text-center font-semibold text-purple-700 transition hover:bg-purple-50 dark:border-purple-700 dark:text-purple-300 dark:hover:bg-purple-900/20">View Score Predictor</Link>
            <Link href="/ap-african-american-studies-daily-question" className="flex-1 rounded-xl border border-purple-300 py-3 text-center font-semibold text-purple-700 transition hover:bg-purple-50 dark:border-purple-700 dark:text-purple-300 dark:hover:bg-purple-900/20">Today&apos;s Question</Link>
          </div>
        </div></div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-violet-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-8 sm:py-12"><div className="mx-auto max-w-2xl">
        <Link href="/courses/ap-african-american-studies" className="mb-4 inline-flex items-center gap-1 text-sm text-purple-600 hover:underline dark:text-purple-400">
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
          AP African American Studies
        </Link>
        <h1 className="mb-2 text-3xl font-bold text-gray-900 sm:text-4xl dark:text-white">✊ AP African American Studies Diagnostic Test</h1>
        <p className="mb-8 text-lg text-gray-600 dark:text-gray-400">Find your strengths and weaknesses across all AP African American Studies units and get a personalized study plan.</p>
        ...
        {lastResult && (
          <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="flex items-center justify-between">
              <div><p className="text-3xl font-bold text-purple-600 dark:text-purple-400">{String(lastResult.estimatedAPScore ?? '—')}/5</p><p className="text-sm text-gray-500 dark:text-gray-400">Estimated AP Score</p></div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <ul className="mb-6 space-y-2 text-sm text-gray-600 dark:text-gray-400">
            {['~12 questions spanning all 4 AP African American Studies units', '35 minute time limit', 'Estimated AP score (1–5) with per-unit breakdown', '3-5 personalized module recommendations', '10 unique forms — each test has different questions'].map(item => (
          ...
          <button onClick={startTest} className="w-full rounded-xl bg-gradient-to-r from-purple-600 to-violet-600 px-6 py-3 font-semibold text-white shadow-lg transition hover:shadow-xl">{lastResult ? 'Take Next Diagnostic' : 'Start Diagnostic Test'}</button>
          <Link href="/ap-african-american-studies-score-predictor" className="mt-3 block text-center text-sm font-medium text-purple-700 transition hover:underline dark:text-purple-300">Prefer a quick estimate? Open the Score Predictor</Link>
          <Link href="/ap-african-american-studies-daily-question" className="mt-1 block text-center text-sm font-medium text-purple-700 transition hover:underline dark:text-purple-300">Need a warm-up first? Try today&apos;s question</Link>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="space-y-3">
            {[
              { step: '1', text: 'Take the diagnostic test — questions cover all 4 AP African American Studies units' },
