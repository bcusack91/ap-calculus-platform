...
import {
  ...
  scoreAPBioDiagnostic,
  pickNextForm,
  type APBioDiagnosticTestData,
...
export default function APBioDiagnosticPage() {
  ...
  const [results, setResults] = useState<APBioDiagnosticResults | null>(null)
  ...
  useEffect(() => {
    if (status === 'authenticated') {
      fetch('/api/ap-bio-diagnostic/history')
  ...
  const handleFinish = useCallback(async () => {
    ...
    const diagnosticResults = scoreAPBioDiagnostic(testData.form, testData.questions, answersRecord)
    setResults(diagnosticResults)
    setPhase('results')

    // Save to API
    try {
      await fetch('/api/ap-bio-diagnostic/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          category: `ap-bio-diagnostic-${testData.form}`,
          results: {
      ...
      const histRes = await fetch('/api/ap-bio-diagnostic/history')
      if (histRes.ok) {
            ...
            <div className="flex items-center justify-between">
              ...
              {currentIndex < testData.questions.length - 1 ? (
                ...
                <button
                  ...
                  className="rounded-xl bg-gradient-to-r from-emerald-600 to-green-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:shadow-lg"
                >
                  Submit Diagnostic
                </button>
              )}
          ...
          {testData && (
                ...
                <div className="space-y-2">
                  {results.recommendedTopics.map((topic, i) => (
                    <Link
                      ...
                      className="flex items-center justify-between rounded-xl border border-emerald-200 bg-white px-4 py-3 transition hover:border-emerald-400 hover:shadow-sm dark:border-emerald-700 dark:bg-gray-800 dark:hover:border-emerald-500 group"
                    >
                      <div className="flex items-center gap-3">
                        <span className="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300">
                          {i + 1}
                        </span>
                        <div>
                          <span className="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-emerald-700 dark:group-hover:text-emerald-400">
                            {topic.name}
                          </span>
            ...
            <div className="mb-8 rounded-2xl border border-blue-200 bg-blue-50 p-5 dark:border-blue-800 dark:bg-blue-900/20">
              ...
              <ol className="space-y-2 text-sm text-blue-700 dark:text-blue-400 list-decimal list-inside">
                ...
                <li>Come back and take the next diagnostic test (Form {results.form + 1 > 10 ? 1 : results.form + 1}) — different questions, same breadth</li>
      ...
      <div className="container py-8 sm:py-12">
        <div className="mx-auto max-w-2xl">
          ...
          <p className="mb-8 text-lg text-gray-600 dark:text-gray-400">
            Find your strengths and weaknesses across all AP Biology units and get a personalized study plan with 3-5 focused modules.
          </p>
          ...
          {lastRecommendedTopics && lastRecommendedTopics.length > 0 && (
            <div className="mb-6 rounded-2xl border-2 border-emerald-300 bg-emerald-50 p-6 dark:border-emerald-700 dark:bg-emerald-900/20">
              <h3 className="mb-1 text-base font-bold text-emerald-800 dark:text-emerald-300">
              ...
              <div className="space-y-2">
                {lastRecommendedTopics.map((topic, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300">
                        {i + 1}
                      </span>
                      <span className="text-sm font-medium text-gray-800 dark:text-gray-200 group-hover:text-emerald-700 dark:group-hover:text-emerald-400">
                        {topic.name}
                      </span>
              ...
              <p className="mt-3 text-xs text-emerald-500 dark:text-emerald-400">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <ul className="mb-6 space-y-2 text-sm text-gray-600 dark:text-gray-400">
              <li className="flex items-start gap-2">
                ...
                ~33 questions spanning all 8 AP Biology units
              </li>
              <li className="flex items-start gap-2">
                <svg className="mt-0.5 h-4 w-4 shrink-0 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              ...
              <li className="flex items-start gap-2">
                ...
                10 rotating forms — each test has different questions
              </li>
            </ul>

            <button
              onClick={startTest}
              className="w-full rounded-xl bg-gradient-to-r from-emerald-600 to-green-600 px-6 py-3 font-semibold text-white shadow-lg transition hover:shadow-xl"
            >
          ...
          <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="space-y-3">
              {[
                { step: '1', text: 'Take the diagnostic test — questions cover all 8 AP Biology units' },
