...
import {
  generateAPEnviroDiagnosticTest,
  scoreAPEnviroDiagnostic,
  pickNextForm,
  type APEnviroDiagnosticTestData,
  type APEnviroDiagnosticResults,
} from '@/data/ap-enviro-diagnostic'
...
export default function APEnviroDiagnosticPage() {
  ...
  const [testData, setTestData] = useState<APEnviroDiagnosticTestData | null>(null)
  const [results, setResults] = useState<APEnviroDiagnosticResults | null>(null)
  ...
  useEffect(() => {
    if (status === 'authenticated') {
      fetch('/api/ap-enviro-diagnostic/history')
  ...
  const startTest = useCallback(() => {
    ...
    const data = generateAPEnviroDiagnosticTest(form)
  ...
  const handleFinish = useCallback(async () => {
    ...
    const diagnosticResults = scoreAPEnviroDiagnostic(testData.form, testData.questions, answersRecord)
    setResults(diagnosticResults)
    setPhase('results')

    try {
      await fetch('/api/ap-enviro-diagnostic/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          category: `ap-enviro-diagnostic-${testData.form}`,
      ...
      const histRes = await fetch('/api/ap-enviro-diagnostic/history')
      ...
      if (challengeToken) {
        const challengeRes = await fetch(`/api/diagnostic-challenges/${challengeToken}/submit`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            category: `ap-enviro-diagnostic-${diagnosticResults.form}`,
  ...
  if (phase === 'testing' && testData) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-lime-50 via-white to-green-50 py-6 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          <div className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="h-2 w-32 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700"><div className="h-full rounded-full bg-lime-500 transition-all" style={{ width: `${((currentIndex + 1) / testData.questions.length) * 100}%` }} /></div>
            </div>
          ...
          <div className="mb-4"><span className="rounded-full bg-lime-100 px-3 py-1 text-xs font-semibold text-lime-700 dark:bg-lime-900/30 dark:text-lime-400">{testData.domains.find(d => d.id === q.domain)?.name ?? q.domain}</span></div>
              ...
              {q.options.map((opt, i) => {
                ...
                return (
                  <button key={i} onClick={() => { const updated = [...answers]; updated[currentIndex] = i; setAnswers(updated) }}
                    className={`w-full rounded-xl border px-4 py-3 text-left text-sm transition ${isSelected ? 'border-lime-500 bg-lime-50 text-lime-800 dark:bg-lime-900/30 dark:text-lime-300 dark:border-lime-500' : 'border-gray-200 text-gray-700 hover:border-lime-300 hover:bg-lime-50/50 dark:border-gray-600 dark:text-gray-300 dark:hover:border-lime-500'}`}>
          ...
          <div className="flex items-center justify-between">
            ...
            {currentIndex < testData.questions.length - 1 ? (
              <button onClick={() => setCurrentIndex(prev => prev + 1)} className="rounded-xl bg-lime-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:bg-lime-700">Next →</button>
            ) : (
              <button onClick={handleFinish} className="rounded-xl bg-gradient-to-r from-lime-600 to-green-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:shadow-lg">Submit Diagnostic</button>
            )}
          </div>
        </div></div>
      </div>
  ...
  if (phase === 'results' && results) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-lime-50 via-white to-green-50 py-8 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          <h2 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">AP Environmental Science Diagnostic Results</h2>

          <div className="mb-8 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800"><p className="text-sm text-gray-500 dark:text-gray-400">Estimated AP Score</p><p className="text-5xl font-black text-lime-600 dark:text-lime-400">{results.estimatedAPScore}</p><p className="text-xs text-gray-500 dark:text-gray-400">out of 5</p></div>
          ...
          <DiagnosticChallengeCard
            category={`ap-enviro-diagnostic-${results.form ?? 1}`}
          ...
          {results.recommendedTopics.length > 0 && !planAccess.loading && !planAccess.canAccess && <DiagnosticPlanPaywall />}
            ...
            <div className="mb-8 rounded-2xl border-2 border-lime-300 bg-lime-50 p-6 dark:border-lime-700 dark:bg-lime-900/20">
              <h3 className="mb-1 text-lg font-bold text-lime-800 dark:text-lime-300">🎯 Your Personalized Study Plan</h3>
              <p className="mb-4 text-sm text-lime-600 dark:text-lime-400">Based on your results, we recommend reviewing these {results.recommendedTopics.length} module{results.recommendedTopics.length > 1 ? 's' : ''}.</p>
              <div className="space-y-2">
                {results.recommendedTopics.map((topic, i) => (
                  <Link key={topic.slug} href={`/topics/${topic.slug}/interactive`} className="flex items-center justify-between rounded-xl border border-lime-200 bg-white px-4 py-3 transition hover:border-lime-400 hover:shadow-sm dark:border-lime-700 dark:bg-gray-800 dark:hover:border-lime-500 group">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-lime-700 dark:group-hover:text-lime-400">{topic.name}</span>
            ...
            <StartStudyPlanButton diagnosticPath="/ap-enviro-diagnostic" className="mb-8" />
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Link href="/courses/ap-environmental-science" className="flex-1 rounded-xl bg-gradient-to-r from-lime-600 to-green-600 py-3 text-center font-semibold text-white shadow transition hover:shadow-lg">Browse All Modules</Link>
            <Link href="/ap-enviro-score-predictor" className="flex-1 rounded-xl border border-lime-300 py-3 text-center font-semibold text-lime-700 transition hover:bg-lime-50 dark:border-lime-700 dark:text-lime-300 dark:hover:bg-lime-900/20">View Score Predictor</Link>
            <Link href="/ap-enviro-daily-question" className="flex-1 rounded-xl border border-lime-300 py-3 text-center font-semibold text-lime-700 transition hover:bg-lime-50 dark:border-lime-700 dark:text-lime-300 dark:hover:bg-lime-900/20">Today&apos;s Question</Link>
          </div>
        </div></div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-lime-50 via-white to-green-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-8 sm:py-12"><div className="mx-auto max-w-2xl">
        <Link href="/courses/ap-environmental-science" className="mb-4 inline-flex items-center gap-1 text-sm text-lime-600 hover:underline dark:text-lime-400">
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
          AP Environmental Science
        </Link>
        <h1 className="mb-2 text-3xl font-bold text-gray-900 sm:text-4xl dark:text-white">🌿 AP Environmental Science Diagnostic Test</h1>
        <p className="mb-8 text-lg text-gray-600 dark:text-gray-400">Find your strengths and weaknesses across all AP Environmental Science units and get a personalized study plan.</p>

        {lastRecommendedTopics && lastRecommendedTopics.length > 0 && (
          <div className="mb-6 rounded-2xl border-2 border-lime-300 bg-lime-50 p-6 dark:border-lime-700 dark:bg-lime-900/20">
            <h3 className="mb-1 text-base font-bold text-lime-800 dark:text-lime-300">🎯 Your Current Study Plan</h3>
            <p className="mb-3 text-sm text-lime-600 dark:text-lime-400">From your last diagnostic — review these modules, then retake the test:</p>
            <div className="space-y-2">
              {lastRecommendedTopics.map((topic, i) => (
                <Link key={topic.slug} href={`/topics/${topic.slug}/interactive`} className="flex items-center justify-between rounded-xl border border-lime-200 bg-white px-4 py-3 transition hover:border-lime-400 hover:shadow-sm dark:border-lime-700 dark:bg-gray-800 group">
                  <div className="flex items-center gap-3">
                    <span className="flex h-6 w-6 items-center justify-center rounded-full bg-lime-100 text-xs font-bold text-lime-700 dark:bg-lime-900/50 dark:text-lime-300">{i + 1}</span>
                    <span className="text-sm font-medium text-gray-800 dark:text-gray-200 group-hover:text-lime-700 dark:group-hover:text-lime-400">{topic.name}</span>
        ...
        {lastResult && (
          <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="flex items-center justify-between">
              <div><p className="text-3xl font-bold text-lime-600 dark:text-lime-400">{String(lastResult.estimatedAPScore ?? '—')}/5</p><p className="text-sm text-gray-500 dark:text-gray-400">Estimated AP Score</p></div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <ul className="mb-6 space-y-2 text-sm text-gray-600 dark:text-gray-400">
            {['~18 questions spanning all 6 AP Environmental Science units', '35 minute time limit', 'Estimated AP score (1–5) with per-unit breakdown', '3-5 personalized module recommendations', '10 unique forms — each test has different questions'].map(item => (
              <li key={item} className="flex items-start gap-2">
                <svg className="mt-0.5 h-4 w-4 shrink-0 text-lime-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                {item}
              </li>
            ))}
          </ul>
          ...
          <Link href="/ap-enviro-score-predictor" className="mt-3 block text-center text-sm font-medium text-lime-700 transition hover:underline dark:text-lime-300">Prefer a quick estimate? Open the Score Predictor</Link>
          <Link href="/ap-enviro-daily-question" className="mt-1 block text-center text-sm font-medium text-lime-700 transition hover:underline dark:text-lime-300">Need a warm-up first? Try today&apos;s question</Link>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="space-y-3">
            {[
              { step: '1', text: 'Take the diagnostic test — questions cover all 6 AP Environmental Science units' },
              ...
              <div key={item.step} className="flex items-start gap-3">
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-lime-100 text-xs font-bold text-lime-700 dark:bg-lime-900/50 dark:text-lime-300">{item.step}</span>
