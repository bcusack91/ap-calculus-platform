...
import {
  generateAPPsychDiagnosticTest,
  scoreAPPsychDiagnostic,
  pickNextForm,
  type APPsychDiagnosticTestData,
  type APPsychDiagnosticResults,
} from '@/data/ap-psychology-diagnostic'
...
export default function APPsychDiagnosticPage() {
  ...
  const [testData, setTestData] = useState<APPsychDiagnosticTestData | null>(null)
  const [results, setResults] = useState<APPsychDiagnosticResults | null>(null)
  ...
  useEffect(() => {
    if (status === 'authenticated') {
      fetch('/api/ap-psych-diagnostic/history')
  ...
  const handleFinish = useCallback(async () => {
    ...
    const diagnosticResults = scoreAPPsychDiagnostic(testData.form, testData.questions, answersRecord)
    setResults(diagnosticResults)
    setPhase('results')

    // Save to API
    try {
      await fetch('/api/ap-psych-diagnostic/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          category: `ap-psych-diagnostic-${testData.form}`,
          results: {
      ...
      // Refresh history so next test picks the right form
      const histRes = await fetch('/api/ap-psych-diagnostic/history')
      if (histRes.ok) {
      ...
      if (challengeToken) {
        const challengeRes = await fetch(`/api/diagnostic-challenges/${challengeToken}/submit`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            category: `ap-psych-diagnostic-${diagnosticResults.form}`,
  ...
  if (status === 'loading') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
  ...
  if (phase === 'testing' && testData) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-purple-50 py-6 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
            ...
            <div className="flex items-center justify-between">
              ...
              {currentIndex < testData.questions.length - 1 ? (
                <button
                  ...
                  className="rounded-xl bg-purple-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:bg-purple-700"
                >
                  Next →
                </button>
              ) : (
                <button
                  ...
                  className="rounded-xl bg-gradient-to-r from-purple-600 to-amber-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:shadow-lg"
                >
                  Submit Diagnostic
                </button>
              )}
  ...
  if (phase === 'results' && results) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-purple-50 py-8 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container">
          <div className="mx-auto max-w-3xl">
            <h2 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">
              AP Psychology Diagnostic Results
            </h2>
                    ...
                    <div className="h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
                      <div
                        className={`h-full rounded-full transition-all ${
                          d.level === 'strong' ? 'bg-green-500' : d.level === 'moderate' ? 'bg-amber-500' : 'bg-red-500'
                        }`}
          ...
          <DiagnosticChallengeCard
            category={`ap-psych-diagnostic-${results.form ?? 1}`}
          ...
          {testData && (
            ...
            {results.recommendedTopics.length > 0 && planAccess.canAccess && (
              <div className="mb-8 rounded-2xl border-2 border-purple-300 bg-purple-50 p-6 dark:border-purple-700 dark:bg-purple-900/20">
                <h3 className="mb-1 text-lg font-bold text-purple-800 dark:text-purple-300">
                ...
                <div className="space-y-2">
                  {results.recommendedTopics.map((topic, i) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="flex h-7 w-7 items-center justify-center rounded-full bg-purple-100 text-xs font-bold text-purple-700 dark:bg-purple-900/50 dark:text-purple-300">
                          {i + 1}
                        </span>
                        <div>
                          <span className="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-purple-700 dark:group-hover:text-purple-400">
                            {topic.name}
                          </span>
            ...
            <div className="mb-8 rounded-2xl border border-blue-200 bg-blue-50 p-5 dark:border-blue-800 dark:bg-blue-900/20">
              ...
              {completedModules > 0 && (
                <p className="mt-3 text-xs text-blue-500 dark:text-blue-400">
                  You&apos;ve taken {completedModules} diagnostic test{completedModules > 1 ? 's' : ''} so far — keep going!
                </p>
              )}
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <button
                ...
                className="flex-1 rounded-xl border-2 border-purple-500 py-3 font-semibold text-purple-600 transition hover:bg-purple-50 dark:text-purple-400 dark:hover:bg-purple-900/20"
              >
                Take Next Diagnostic
              </button>
              <Link
                href="/courses/ap-psychology"
                className="flex-1 rounded-xl bg-gradient-to-r from-purple-600 to-amber-600 py-3 text-center font-semibold text-white shadow transition hover:shadow-lg"
              >
                Browse All AP Psych Modules
              </Link>
              <Link
                href="/ap-psych-score-predictor"
                className="flex-1 rounded-xl border border-purple-300 py-3 text-center font-semibold text-purple-700 transition hover:bg-purple-50 dark:border-purple-700 dark:text-purple-300 dark:hover:bg-purple-900/20"
              >
      ...
      <div className="container py-8 sm:py-12">
        <div className="mx-auto max-w-2xl">
          ...
          <h1 className="mb-2 text-3xl font-bold text-gray-900 sm:text-4xl dark:text-white">
            🧠 AP Psychology Diagnostic Test
          </h1>
          <p className="mb-8 text-lg text-gray-600 dark:text-gray-400">
            Find your strengths and weaknesses across all AP Psychology units and get a personalized study plan with 3-5 focused modules.
          </p>

          {/* Active study plan from last diagnostic */}
          {lastRecommendedTopics && lastRecommendedTopics.length > 0 && (
            <div className="mb-6 rounded-2xl border-2 border-purple-300 bg-purple-50 p-6 dark:border-purple-700 dark:bg-purple-900/20">
              <h3 className="mb-1 text-base font-bold text-purple-800 dark:text-purple-300">
                🎯 Your Current Study Plan
              </h3>
              <p className="mb-3 text-sm text-purple-600 dark:text-purple-400">
                From your last diagnostic — review these modules, then retake the test:
              </p>
              <div className="space-y-2">
                {lastRecommendedTopics.map((topic, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="flex h-6 w-6 items-center justify-center rounded-full bg-purple-100 text-xs font-bold text-purple-700 dark:bg-purple-900/50 dark:text-purple-300">
                        {i + 1}
                      </span>
                      <span className="text-sm font-medium text-gray-800 dark:text-gray-200 group-hover:text-purple-700 dark:group-hover:text-purple-400">
                        {topic.name}
                      </span>
              ...
              <p className="mt-3 text-xs text-purple-500 dark:text-purple-400">
                Once you&apos;ve reviewed these, retake the diagnostic below to get updated recommendations.
              </p>
            </div>
          )}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-3xl font-bold text-purple-600 dark:text-purple-400">
                    {String(lastResult.estimatedAPScore ?? '—')}/5
                  </p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Estimated AP Score</p>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-400">
                    Form {String(lastResult.form ?? '—')} · {new Date(history[0].createdAt).toLocaleDateString()}<br /><Link href={`/diagnostic-review/${history[0].id}`} className="mt-1 inline-block text-xs font-semibold text-purple-600 hover:underline dark:text-purple-400" onClick={(e) => e.stopPropagation()}>Review past attempt →</Link>
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <ul className="mb-6 space-y-2 text-sm text-gray-600 dark:text-gray-400">
              <li className="flex items-start gap-2">
                ...
                ~33 questions spanning all 11 AP Psychology units
              </li>
              <li className="flex items-start gap-2">
                <svg className="mt-0.5 h-4 w-4 shrink-0 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              ...
              <li className="flex items-start gap-2">
                ...
                Estimated AP score (1–5) with per-unit breakdown
              </li>
              <li className="flex items-start gap-2">
                <svg className="mt-0.5 h-4 w-4 shrink-0 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                </svg>
                3-5 personalized module recommendations to focus your studying
              </li>
              <li className="flex items-start gap-2">
                <svg className="mt-0.5 h-4 w-4 shrink-0 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            ...
            <button
              onClick={startTest}
              className="w-full rounded-xl bg-gradient-to-r from-purple-600 to-amber-600 px-6 py-3 font-semibold text-white shadow-lg transition hover:shadow-xl"
            >
              {lastResult ? 'Take Next Diagnostic' : 'Start Diagnostic Test'}
            </button>
            <Link href="/ap-psych-score-predictor" className="mt-3 block text-center text-sm font-medium text-purple-700 transition hover:underline dark:text-purple-300">Prefer a quick estimate? Open the AP Psychology Score Predictor</Link>
            <Link href="/ap-psych-daily-question" className="mt-1 block text-center text-sm font-medium text-purple-700 transition hover:underline dark:text-purple-300">Need a warm-up first? Try today&apos;s AP Psychology question</Link>
          </div>
          ...
          <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <h3 className="mb-3 text-base font-semibold text-gray-800 dark:text-gray-200">
              🔄 How the Diagnostic Cycle Works
            </h3>
            <div className="space-y-3">
              {[
                { step: '1', text: 'Take the diagnostic test — questions cover all 11 AP Psychology units' },
