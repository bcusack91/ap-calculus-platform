...
import {
  ...
  scoreAlgebra2Diagnostic,
  pickNextForm,
  TOTAL_FORMS,
  type Algebra2DiagnosticTestData,
...
export default function Algebra2DiagnosticPage() {
  ...
  const [results, setResults] = useState<Algebra2DiagnosticResults | null>(null)
  ...
  useEffect(() => {
    if (status === 'authenticated') {
      fetch('/api/algebra2-diagnostic/history')
  ...
  if (phase === 'testing' && testData) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-amber-50 py-6 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          <div className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="h-2 w-32 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700"><div className="h-full rounded-full bg-orange-500 transition-all" style={{ width: `${((currentIndex + 1) / testData.questions.length) * 100}%` }} /></div>
            </div>
          ...
          <div className="mb-4"><span className="rounded-full bg-orange-100 px-3 py-1 text-xs font-semibold text-orange-700 dark:bg-orange-900/30 dark:text-orange-400">{testData.domains.find(d => d.id === q.domain)?.name ?? q.domain}</span></div>
              ...
              {q.options.map((opt, i) => {
                ...
                return (
                  <button key={i} onClick={() => { if (isEliminated) return; const updated = [...answers]; updated[currentIndex] = i; setAnswers(updated) }}
                    className={`w-full rounded-xl border px-4 py-3 text-left text-sm transition ${isSelected ? 'border-orange-500 bg-orange-50 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300 dark:border-orange-500' : 'border-gray-200 text-gray-700 hover:border-orange-300 hover:bg-orange-50/50 dark:border-gray-600 dark:text-gray-300 dark:hover:border-orange-500'}`}>
          ...
          <div className="flex items-center justify-between">
            ...
            {currentIndex < testData.questions.length - 1 ? (
              <button onClick={() => setCurrentIndex(prev => prev + 1)} className="rounded-xl bg-orange-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:bg-orange-700">Next →</button>
            ) : (
              <button onClick={handleFinish} className="rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:shadow-lg">Submit Diagnostic</button>
            )}
          </div>
        </div></div>
      </div>
  ...
  if (phase === 'results' && results) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-amber-50 py-8 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          <h2 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Algebra 2 Diagnostic Results</h2>
          <div className="mb-8 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800"><p className="text-sm text-gray-500 dark:text-gray-400">Level</p><p className="text-3xl font-black text-orange-600 dark:text-orange-400">{results.estimatedLevel}</p></div>
          ...
          {results.recommendedTopics.length > 0 && !planAccess.loading && !planAccess.canAccess && <DiagnosticPlanPaywall />}
            ...
            <div className="mb-8 rounded-2xl border-2 border-orange-300 bg-orange-50 p-6 dark:border-orange-700 dark:bg-orange-900/20">
              <h3 className="mb-1 text-lg font-bold text-orange-800 dark:text-orange-300">🎯 Your Personalized Study Plan</h3>
              <p className="mb-4 text-sm text-orange-600 dark:text-orange-400">Based on your results, review these {results.recommendedTopics.length} topic{results.recommendedTopics.length > 1 ? 's' : ''}.</p>
              <div className="space-y-2">
                {results.recommendedTopics.map((topic, i) => (
                  <Link key={topic.slug} href={`/topics/${topic.slug}/interactive`} className="flex items-center justify-between rounded-xl border border-orange-200 bg-white px-4 py-3 transition hover:border-orange-400 hover:shadow-sm dark:border-orange-700 dark:bg-gray-800 dark:hover:border-orange-500 group">
                    <div className="flex items-center gap-3">
                      <span className="flex h-7 w-7 items-center justify-center rounded-full bg-orange-100 text-xs font-bold text-orange-700 dark:bg-orange-900/50 dark:text-orange-300">{i + 1}</span>
                      <div><span className="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-orange-700 dark:group-hover:text-orange-400">{topic.name}</span><span className={`ml-2 text-xs px-2 py-0.5 rounded-full ${topic.priority === 'high' ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' : 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400'}`}>{topic.priority === 'high' ? 'High Priority' : 'Medium Priority'}</span></div>
                    </div>
            ...
            <StartStudyPlanButton diagnosticPath="/algebra2-diagnostic" className="mb-8" />
          <div className="mb-8 rounded-2xl border border-orange-200 bg-orange-50 p-5 dark:border-orange-800 dark:bg-orange-900/20">
            <h4 className="mb-2 font-semibold text-orange-800 dark:text-orange-300">🔄 How This Cycle Works</h4>
            <ol className="space-y-2 text-sm text-orange-700 dark:text-orange-400 list-decimal list-inside">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-amber-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-8 sm:py-12"><div className="mx-auto max-w-2xl">
        <Link href="/courses/algebra-2" className="mb-4 inline-flex items-center gap-1 text-sm text-orange-600 hover:underline dark:text-orange-400">
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>Algebra 2
        </Link>
        <h1 className="mb-2 text-3xl font-bold text-gray-900 sm:text-4xl dark:text-white">📊 Algebra 2 Diagnostic Test</h1>
        <p className="mb-8 text-lg text-gray-600 dark:text-gray-400">Identify your strengths and weaknesses across all Algebra 2 domains and get a personalized study plan.</p>
        {lastRecommendedTopics && lastRecommendedTopics.length > 0 && (
          <div className="mb-6 rounded-2xl border-2 border-orange-300 bg-orange-50 p-6 dark:border-orange-700 dark:bg-orange-900/20">
            <h3 className="mb-1 text-base font-bold text-orange-800 dark:text-orange-300">🎯 Your Current Study Plan</h3>
            <p className="mb-3 text-sm text-orange-600 dark:text-orange-400">From your last diagnostic — review these topics, then retake the test:</p>
            <div className="space-y-2">
              {lastRecommendedTopics.map((topic, i) => (
                <Link key={topic.slug} href={`/topics/${topic.slug}/interactive`} className="flex items-center justify-between rounded-xl border border-orange-200 bg-white px-4 py-3 transition hover:border-orange-400 hover:shadow-sm dark:border-orange-700 dark:bg-gray-800 group">
                  <div className="flex items-center gap-3"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-orange-100 text-xs font-bold text-orange-700 dark:bg-orange-900/50 dark:text-orange-300">{i + 1}</span><span className="text-sm font-medium text-gray-800 dark:text-gray-200 group-hover:text-orange-700 dark:group-hover:text-orange-400">{topic.name}</span><span className={`text-xs px-2 py-0.5 rounded-full ${topic.priority === 'high' ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' : 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400'}`}>{topic.priority === 'high' ? 'High' : 'Medium'}</span></div>
                  <span className="text-orange-500 group-hover:translate-x-1 transition-transform">→</span>
                </Link>
              ))}
            </div>
        ...
        {lastResult && (
          <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="flex items-center justify-between">
              <div><p className="text-3xl font-bold text-orange-600 dark:text-orange-400">{String(lastResult.estimatedLevel ?? '—')}</p><p className="text-sm text-gray-500 dark:text-gray-400">Estimated Level</p></div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <ul className="mb-6 space-y-2 text-sm text-gray-600 dark:text-gray-400">
            {['~30 questions spanning 6 Algebra 2 domains', '40 minute time limit', 'Proficiency level assessment with per-domain breakdown', 'Personalized topic recommendations', `${TOTAL_FORMS} unique forms with different questions each time`].map(item => (
              <li key={item} className="flex items-start gap-2"><svg className="mt-0.5 h-4 w-4 shrink-0 text-orange-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>{item}</li>
            ))}
          </ul>
        ...
        <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="space-y-3">
            {[{ step: '1', text: 'Take the diagnostic test — questions cover all 6 Algebra 2 domains' }, { step: '2', text: 'Get your results and topic recommendations based on what you missed' }, { step: '3', text: 'Study those specific topics (lessons, practice problems)' }, { step: '4', text: 'Retake the diagnostic — a different form with fresh questions' }, { step: '5', text: 'Repeat until you reach the Advanced level!' }].map(item => (
              <div key={item.step} className="flex items-start gap-3"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-orange-100 text-xs font-bold text-orange-700 dark:bg-orange-900/50 dark:text-orange-300">{item.step}</span><p className="text-sm text-gray-600 dark:text-gray-400">{item.text}</p></div>
            ))}
          </div>
        </div>
        {history.length > 1 && (
          <div className="mt-6"><h3 className="mb-3 text-sm font-medium text-gray-500 uppercase dark:text-gray-400">Previous Attempts</h3>
            <div className="space-y-2">{history.slice(1, 8).map(h => { const parsed = h.results as Record<string, unknown>; return (<Link key={h.id} href={`/diagnostic-review/${h.id}`} className="flex items-center justify-between rounded-lg bg-gray-50 p-3 dark:bg-gray-700/50 cursor-pointer transition hover:bg-gray-100 dark:hover:bg-gray-600/60 hover:shadow-sm"><div className="flex items-center gap-3"><span className="text-sm font-semibold text-gray-700 dark:text-gray-300">Level: {String(parsed?.estimatedLevel ?? '—')}</span><span className="text-xs text-gray-500 dark:text-gray-400">Form {String(parsed?.form ?? '?')}</span></div><span className="text-xs text-gray-400">{new Date(h.createdAt).toLocaleDateString()}</span></Link>) })}</div>
          </div>
        )}
      </div></div>
    </div>
