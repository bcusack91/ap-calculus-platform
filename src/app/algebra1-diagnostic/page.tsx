...
import {
  generateAlgebra1DiagnosticTest,
  scoreAlgebra1Diagnostic,
  pickNextForm,
  TOTAL_FORMS,
  type Algebra1DiagnosticTestData,
  type Algebra1DiagnosticResults,
} from '@/data/algebra1-diagnostic'
...
export default function Algebra1DiagnosticPage() {
  ...
  const [testData, setTestData] = useState<Algebra1DiagnosticTestData | null>(null)
  const [results, setResults] = useState<Algebra1DiagnosticResults | null>(null)
  ...
  useEffect(() => {
    if (status === 'unauthenticated') router.push('/auth/signin?callbackUrl=/algebra1-diagnostic')
  }, [status, router])

  useEffect(() => {
    if (status === 'authenticated') {
      fetch('/api/algebra1-diagnostic/history')
  ...
  const startTest = useCallback(() => {
    ...
    const data = generateAlgebra1DiagnosticTest(form)
  ...
  const handleFinish = useCallback(async () => {
    ...
    const diagnosticResults = scoreAlgebra1Diagnostic(testData.form, testData.questions, answersRecord)
    setResults(diagnosticResults); setPhase('results')
    try {
      await fetch('/api/algebra1-diagnostic/submit', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ category: `algebra1-diagnostic-${testData.form}`, results: { form: diagnosticResults.form, totalCorrect: diagnosticResults.totalCorrect, totalQuestions: diagnosticResults.totalQuestions, percentage: diagnosticResults.percentage, estimatedLevel: diagnosticResults.estimatedLevel, domains: diagnosticResults.domains, recommendedTopics: diagnosticResults.recommendedTopics }, weakAreas: diagnosticResults.weakAreas, strengths: diagnosticResults.strengths.join(', ') }),
      })
      const histRes = await fetch('/api/algebra1-diagnostic/history')
      ...
      if (challengeToken) {
        const challengeRes = await fetch(`/api/diagnostic-challenges/${challengeToken}/submit`, {
          ...
          body: JSON.stringify({
            category: `algebra1-diagnostic-${diagnosticResults.form}`,
        ...
        <div className="container"><div className="mx-auto max-w-3xl">
          <div className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="h-2 w-32 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700"><div className="h-full rounded-full bg-green-500 transition-all" style={{ width: `${((currentIndex + 1) / testData.questions.length) * 100}%` }} /></div>
            </div>
          ...
          <div className="mb-4"><span className="rounded-full bg-green-100 px-3 py-1 text-xs font-semibold text-green-700 dark:bg-green-900/30 dark:text-green-400">{testData.domains.find(d => d.id === q.domain)?.name ?? q.domain}</span></div>
              ...
              {q.options.map((opt, i) => {
                ...
                return (
                  <button key={i} onClick={() => { if (isEliminated) return; const updated = [...answers]; updated[currentIndex] = i; setAnswers(updated) }}
                    className={`w-full rounded-xl border px-4 py-3 text-left text-sm transition ${isSelected ? 'border-green-500 bg-green-50 text-green-800 dark:bg-green-900/30 dark:text-green-300 dark:border-green-500' : 'border-gray-200 text-gray-700 hover:border-green-300 hover:bg-green-50/50 dark:border-gray-600 dark:text-gray-300 dark:hover:border-green-500'}`}>
          ...
          <div className="flex items-center justify-between">
            ...
            {currentIndex < testData.questions.length - 1 ? (
              <button onClick={() => setCurrentIndex(prev => prev + 1)} className="rounded-xl bg-green-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:bg-green-700">Next →</button>
            ) : (
              <button onClick={handleFinish} className="rounded-xl bg-gradient-to-r from-green-600 to-teal-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:shadow-lg">Submit Diagnostic</button>
            )}
          </div>
        </div></div>
      </div>
  ...
  if (phase === 'results' && results) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-teal-50 py-8 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          <h2 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Algebra 1 Diagnostic Results</h2>
          <div className="mb-8 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800"><p className="text-sm text-gray-500 dark:text-gray-400">Level</p><p className="text-3xl font-black text-green-600 dark:text-green-400">{results.estimatedLevel}</p></div>
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800"><p className="text-sm text-gray-500 dark:text-gray-400">Correct</p><p className="text-4xl font-black text-teal-600 dark:text-teal-400">{results.totalCorrect}/{results.totalQuestions}</p><p className="text-xs text-gray-500 dark:text-gray-400">{results.percentage}%</p></div>
          ...
          <DiagnosticChallengeCard
            category={`algebra1-diagnostic-${results.form ?? 1}`}
          ...
          {results.recommendedTopics.length > 0 && (
            <div className="mb-8 rounded-2xl border-2 border-green-300 bg-green-50 p-6 dark:border-green-700 dark:bg-green-900/20">
              ...
              <p className="mb-4 text-sm text-green-600 dark:text-green-400">Based on your results, review these {results.recommendedTopics.length} topic{results.recommendedTopics.length > 1 ? 's' : ''}.</p>
              <div className="space-y-2">
                {results.recommendedTopics.map((topic, i) => (
                  <Link key={topic.slug} href={`/topics/${topic.slug}/interactive`} className="flex items-center justify-between rounded-xl border border-green-200 bg-white px-4 py-3 transition hover:border-green-400 hover:shadow-sm dark:border-green-700 dark:bg-gray-800 dark:hover:border-green-500 group">
                    <div className="flex items-center gap-3">
                      <span className="flex h-7 w-7 items-center justify-center rounded-full bg-green-100 text-xs font-bold text-green-700 dark:bg-green-900/50 dark:text-green-300">{i + 1}</span>
                      <div><span className="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-green-700 dark:group-hover:text-green-400">{topic.name}</span><span className={`ml-2 text-xs px-2 py-0.5 rounded-full ${topic.priority === 'high' ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' : 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400'}`}>{topic.priority === 'high' ? 'High Priority' : 'Medium Priority'}</span></div>
                    </div>
          ...
          <div className="mb-8 rounded-2xl border border-green-200 bg-green-50 p-5 dark:border-green-800 dark:bg-green-900/20">
            ...
            {completedModules > 0 && <p className="mt-3 text-xs text-green-500 dark:text-green-400">You&apos;ve taken {completedModules} diagnostic test{completedModules > 1 ? 's' : ''} so far — keep going!</p>}
          </div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <button onClick={() => { setResults(null); setTestData(null); startTest() }} className="flex-1 rounded-xl border-2 border-green-500 py-3 font-semibold text-green-600 transition hover:bg-green-50 dark:text-green-400 dark:hover:bg-green-900/20">Take Next Diagnostic</button>
            <Link href="/courses/algebra-1" className="flex-1 rounded-xl bg-gradient-to-r from-green-600 to-teal-600 py-3 text-center font-semibold text-white shadow transition hover:shadow-lg">Browse Algebra 1 Topics</Link>
            <Link href="/algebra1-score-predictor" className="flex-1 rounded-xl border border-green-300 py-3 text-center font-semibold text-green-700 transition hover:bg-green-50 dark:border-green-700 dark:text-green-300 dark:hover:bg-green-900/20">View Score Predictor</Link>
            <Link href="/algebra1-daily-question" className="flex-1 rounded-xl border border-green-300 py-3 text-center font-semibold text-green-700 transition hover:bg-green-50 dark:border-green-700 dark:text-green-300 dark:hover:bg-green-900/20">Today&apos;s Question</Link>
          </div>
        </div></div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-teal-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-8 sm:py-12"><div className="mx-auto max-w-2xl">
        <Link href="/courses/algebra-1" className="mb-4 inline-flex items-center gap-1 text-sm text-green-600 hover:underline dark:text-green-400">
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>Algebra 1
        </Link>
        <h1 className="mb-2 text-3xl font-bold text-gray-900 sm:text-4xl dark:text-white">🔢 Algebra 1 Diagnostic Test</h1>
        <p className="mb-8 text-lg text-gray-600 dark:text-gray-400">Identify your strengths and weaknesses across all Algebra 1 domains and get a personalized study plan.</p>
        ...
        {lastRecommendedTopics && lastRecommendedTopics.length > 0 && (
          <div className="mb-6 rounded-2xl border-2 border-green-300 bg-green-50 p-6 dark:border-green-700 dark:bg-green-900/20">
            ...
            <p className="mb-3 text-sm text-green-600 dark:text-green-400">From your last diagnostic — review these topics, then retake the test:</p>
            <div className="space-y-2">
              {lastRecommendedTopics.map((topic, i) => (
                <Link key={topic.slug} href={`/topics/${topic.slug}/interactive`} className="flex items-center justify-between rounded-xl border border-green-200 bg-white px-4 py-3 transition hover:border-green-400 hover:shadow-sm dark:border-green-700 dark:bg-gray-800 group">
                  <div className="flex items-center gap-3"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-green-100 text-xs font-bold text-green-700 dark:bg-green-900/50 dark:text-green-300">{i + 1}</span><span className="text-sm font-medium text-gray-800 dark:text-gray-200 group-hover:text-green-700 dark:group-hover:text-green-400">{topic.name}</span><span className={`text-xs px-2 py-0.5 rounded-full ${topic.priority === 'high' ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' : 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400'}`}>{topic.priority === 'high' ? 'High' : 'Medium'}</span></div>
                  <span className="text-green-500 group-hover:translate-x-1 transition-transform">→</span>
                </Link>
              ))}
            </div>
        ...
        {lastResult && (
          <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="flex items-center justify-between">
              <div><p className="text-3xl font-bold text-green-600 dark:text-green-400">{String(lastResult.estimatedLevel ?? '—')}</p><p className="text-sm text-gray-500 dark:text-gray-400">Estimated Level</p></div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <ul className="mb-6 space-y-2 text-sm text-gray-600 dark:text-gray-400">
            {['~30 questions spanning 6 Algebra 1 domains', '35 minute time limit', 'Proficiency level assessment with per-domain breakdown', 'Personalized topic recommendations', `${TOTAL_FORMS} unique forms with different questions each time`].map(item => (
              <li key={item} className="flex items-start gap-2"><svg className="mt-0.5 h-4 w-4 shrink-0 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>{item}</li>
            ))}
          </ul>
          <button onClick={startTest} className="w-full rounded-xl bg-gradient-to-r from-green-600 to-teal-600 px-6 py-3 font-semibold text-white shadow-lg transition hover:shadow-xl">{lastResult ? 'Take Next Diagnostic' : 'Start Diagnostic Test'}</button>
          <Link href="/algebra1-score-predictor" className="mt-3 block text-center text-sm font-medium text-green-700 transition hover:underline dark:text-green-300">Prefer a quick estimate? Open the Algebra 1 Score Predictor</Link>
          <Link href="/algebra1-daily-question" className="mt-1 block text-center text-sm font-medium text-green-700 transition hover:underline dark:text-green-300">Need a warm-up first? Try today&apos;s Algebra 1 question</Link>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="space-y-3">
            {[
              { step: '1', text: 'Take the diagnostic test — questions cover all 6 Algebra 1 domains' },
