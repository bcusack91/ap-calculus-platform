...
import {
  generateAPHumanGeoDiagnosticTest,
  scoreAPHumanGeoDiagnostic,
  pickNextForm,
  type APHumanGeoDiagnosticTestData,
  type APHumanGeoDiagnosticResults,
} from '@/data/ap-human-geography-diagnostic'
...
export default function APHumanGeoDiagnosticPage() {
  ...
  const [testData, setTestData] = useState<APHumanGeoDiagnosticTestData | null>(null)
  const [results, setResults] = useState<APHumanGeoDiagnosticResults | null>(null)
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.push('/auth/signin?callbackUrl=/ap-human-geo-diagnostic')
    }
  }, [status, router])

  useEffect(() => {
    if (status === 'authenticated') {
      fetch('/api/ap-human-geo-diagnostic/history')
  ...
  const startTest = useCallback(() => {
    ...
    const data = generateAPHumanGeoDiagnosticTest(form)
  ...
  const handleFinish = useCallback(async () => {
    ...
    const diagnosticResults = scoreAPHumanGeoDiagnostic(testData.form, testData.questions, answersRecord)
    setResults(diagnosticResults)
    setPhase('results')

    try {
      await fetch('/api/ap-human-geo-diagnostic/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          category: `ap-human-geo-diagnostic-${testData.form}`,
          results: {
            review: { questions: testData.questions, answers, domainNames: Object.fromEntries(testData.domains.map(d => [d.id, d.name])) }, form: diagnosticResults.form,
      ...
      const histRes = await fetch('/api/ap-human-geo-diagnostic/history')
      if (histRes.ok) {
      ...
      if (challengeToken) {
        const challengeRes = await fetch(`/api/diagnostic-challenges/${challengeToken}/submit`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            category: `ap-human-geo-diagnostic-${diagnosticResults.form}`,
        ...
        <div className="container">
          <div className="mx-auto max-w-3xl">
              ...
              <div className="space-y-2">
                {q.options.map((opt, i) => {
                  ...
                  return (
                    <button
                      key={i}
                      onClick={() => { const updated = [...answers]; updated[currentIndex] = i; setAnswers(updated) }}
                        ...
                        <span
                          ...
                          onClick={(e) => {
                            ...
                            setEliminatedOptions(prev => {
                              ...
                              if (qSet.has(i)) { qSet.delete(i) } else {
                                qSet.add(i)
                                if (answers[currentIndex] === i) { const updated = [...answers]; updated[currentIndex] = null; setAnswers(updated) }
                              }
                          ...
                          onKeyDown={(e) => {
                            if (e.key === 'Enter' || e.key === ' ') {
                              ...
                              setEliminatedOptions(prev => {
                                const next = prev.map(set => new Set(set))
                                const qSet = new Set(next[currentIndex] ?? [])
                                if (qSet.has(i)) qSet.delete(i); else qSet.add(i)
                                next[currentIndex] = qSet
                                return next
                              })
                            }
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => { const updated = [...answers]; updated[currentIndex] = null; setAnswers(updated); if (currentIndex < testData.questions.length - 1) { setCurrentIndex(prev => prev + 1) } else { handleFinish() } }}
              ...
              >I Would Be Guessing</button>
              {currentIndex < testData.questions.length - 1 ? (
                <button onClick={() => setCurrentIndex(prev => prev + 1)} className="rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:bg-emerald-700">Next →</button>
              ) : (
                <button onClick={handleFinish} className="rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:shadow-lg">Submit Diagnostic</button>
              )}
            </div>
          </div>
        </div>
  ...
  if (phase === 'results' && results) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 py-8 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container">
          <div className="mx-auto max-w-3xl">
            <h2 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">AP Human Geography Diagnostic Results</h2>
            ...
            <div className="mb-8 grid gap-4 sm:grid-cols-3">
              <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800">
                ...
                <p className="text-5xl font-black text-emerald-600 dark:text-emerald-400">{results.estimatedAPScore}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">out of 5</p>
              </div>
              <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800">
                <p className="text-sm text-gray-500 dark:text-gray-400">Correct</p>
                <p className="text-4xl font-black text-amber-600 dark:text-amber-400">{results.totalCorrect}/{results.totalQuestions}</p>
          ...
          <DiagnosticChallengeCard
            category={`ap-human-geo-diagnostic-${results.form ?? 1}`}
          ...
          {testData && (
            ...
            {results.recommendedTopics.length > 0 && planAccess.canAccess && (
              <div className="mb-8 rounded-2xl border-2 border-emerald-300 bg-emerald-50 p-6 dark:border-emerald-700 dark:bg-emerald-900/20">
                <h3 className="mb-1 text-lg font-bold text-emerald-800 dark:text-emerald-300">🎯 Your Personalized Study Plan</h3>
                <p className="mb-4 text-sm text-emerald-600 dark:text-emerald-400">Based on your results, we recommend reviewing these {results.recommendedTopics.length} module{results.recommendedTopics.length > 1 ? 's' : ''}.</p>
                <div className="space-y-2">
                  {results.recommendedTopics.map((topic, i) => (
                    <Link key={topic.slug} href={`/topics/${topic.slug}/interactive`} className="flex items-center justify-between rounded-xl border border-emerald-200 bg-white px-4 py-3 transition hover:border-emerald-400 hover:shadow-sm dark:border-emerald-700 dark:bg-gray-800 dark:hover:border-emerald-500 group">
                      <div className="flex items-center gap-3">
                        <span className="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300">{i + 1}</span>
                        <div>
                          <span className="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-emerald-700 dark:group-hover:text-emerald-400">{topic.name}</span>
            ...
            <StartStudyPlanButton diagnosticPath="/ap-human-geo-diagnostic" className="mb-8" />
            ...
            <div className="mb-8 rounded-2xl border border-blue-200 bg-blue-50 p-5 dark:border-blue-800 dark:bg-blue-900/20">
              ...
              {completedModules > 0 && (
                <p className="mt-3 text-xs text-blue-500 dark:text-blue-400">You&apos;ve taken {completedModules} diagnostic test{completedModules > 1 ? 's' : ''} so far — keep going!</p>
              )}
            </div>

            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <button onClick={() => { setResults(null); setTestData(null); startTest() }} className="flex-1 rounded-xl border-2 border-emerald-500 py-3 font-semibold text-emerald-600 transition hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-900/20">Take Next Diagnostic</button>
              <Link href="/courses/ap-human-geography" className="flex-1 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 py-3 text-center font-semibold text-white shadow transition hover:shadow-lg">Browse All Modules</Link>
              <Link href="/ap-human-geo-score-predictor" className="flex-1 rounded-xl border border-emerald-300 py-3 text-center font-semibold text-emerald-700 transition hover:bg-emerald-50 dark:border-emerald-700 dark:text-emerald-300 dark:hover:bg-emerald-900/20">View Score Predictor</Link>
              <Link href="/ap-human-geo-daily-question" className="flex-1 rounded-xl border border-emerald-300 py-3 text-center font-semibold text-emerald-700 transition hover:bg-emerald-50 dark:border-emerald-700 dark:text-emerald-300 dark:hover:bg-emerald-900/20">Today&apos;s Question</Link>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-8 sm:py-12">
        <div className="mx-auto max-w-2xl">
          <Link href="/courses/ap-human-geography" className="mb-4 inline-flex items-center gap-1 text-sm text-emerald-600 hover:underline dark:text-emerald-400">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
            AP Human Geography
          </Link>
          <h1 className="mb-2 text-3xl font-bold text-gray-900 sm:text-4xl dark:text-white">🌍 AP Human Geography Diagnostic Test</h1>
          <p className="mb-8 text-lg text-gray-600 dark:text-gray-400">Find your strengths and weaknesses across all AP Human Geography units and get a personalized study plan.</p>
          ...
          {lastRecommendedTopics && lastRecommendedTopics.length > 0 && (
            <div className="mb-6 rounded-2xl border-2 border-emerald-300 bg-emerald-50 p-6 dark:border-emerald-700 dark:bg-emerald-900/20">
              <h3 className="mb-1 text-base font-bold text-emerald-800 dark:text-emerald-300">🎯 Your Current Study Plan</h3>
              <p className="mb-3 text-sm text-emerald-600 dark:text-emerald-400">From your last diagnostic — review these modules, then retake the test:</p>
              <div className="space-y-2">
                {lastRecommendedTopics.map((topic, i) => (
                  <Link key={topic.slug} href={`/topics/${topic.slug}/interactive`} className="flex items-center justify-between rounded-xl border border-emerald-200 bg-white px-4 py-3 transition hover:border-emerald-400 hover:shadow-sm dark:border-emerald-700 dark:bg-gray-800 group">
                    <div className="flex items-center gap-3">
                      <span className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300">{i + 1}</span>
              ...
              <p className="mt-3 text-xs text-emerald-500 dark:text-emerald-400">Once you&apos;ve reviewed these, retake the diagnostic below to get updated recommendations.</p>
            </div>
          )}
          ...
          {lastResult && (
            <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-3xl font-bold text-emerald-600 dark:text-emerald-400">{String(lastResult.estimatedAPScore ?? '—')}/5</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Estimated AP Score</p>
                </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <ul className="mb-6 space-y-2 text-sm text-gray-600 dark:text-gray-400">
              {['~21 questions spanning all 7 AP Human Geography units', '35 minute time limit', 'Estimated AP score (1–5) with per-unit breakdown', '3-5 personalized module recommendations', '10 unique forms — each test has different questions'].map(item => (
                <li key={item} className="flex items-start gap-2">
                  <svg className="mt-0.5 h-4 w-4 shrink-0 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                  {item}
                </li>
              ))}
            </ul>
            <button onClick={startTest} className="w-full rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-3 font-semibold text-white shadow-lg transition hover:shadow-xl">{lastResult ? 'Take Next Diagnostic' : 'Start Diagnostic Test'}</button>
            <Link href="/ap-human-geo-score-predictor" className="mt-3 block text-center text-sm font-medium text-emerald-700 transition hover:underline dark:text-emerald-300">Prefer a quick estimate? Open the Score Predictor</Link>
            <Link href="/ap-human-geo-daily-question" className="mt-1 block text-center text-sm font-medium text-emerald-700 transition hover:underline dark:text-emerald-300">Need a warm-up first? Try today&apos;s question</Link>
          </div>
          ...
          <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="space-y-3">
              {[
                { step: '1', text: 'Take the diagnostic test — questions cover all 7 AP Human Geography units' },
                ...
                <div key={item.step} className="flex items-start gap-3">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300">{item.step}</span>
