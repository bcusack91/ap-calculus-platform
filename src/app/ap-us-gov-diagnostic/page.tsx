...
import {
  generateAPUsGovDiagnosticTest,
  scoreAPUsGovDiagnostic,
  pickNextForm,
  type APUsGovDiagnosticTestData,
  type APUsGovDiagnosticResults,
} from '@/data/ap-us-gov-diagnostic'
...
export default function APUsGovDiagnosticPage() {
  ...
  const [testData, setTestData] = useState<APUsGovDiagnosticTestData | null>(null)
  const [results, setResults] = useState<APUsGovDiagnosticResults | null>(null)
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.push('/auth/signin?callbackUrl=/ap-us-gov-diagnostic')
    }
  }, [status, router])

  useEffect(() => {
    if (status === 'authenticated') {
      fetch('/api/ap-us-gov-diagnostic/history')
  ...
  const startTest = useCallback(() => {
    ...
    const data = generateAPUsGovDiagnosticTest(form)
  ...
  const handleFinish = useCallback(async () => {
    ...
    const diagnosticResults = scoreAPUsGovDiagnostic(testData.form, testData.questions, answersRecord)
    setResults(diagnosticResults)
    setPhase('results')

    try {
      await fetch('/api/ap-us-gov-diagnostic/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          category: `ap-us-gov-diagnostic-${testData.form}`,
      ...
      const histRes = await fetch('/api/ap-us-gov-diagnostic/history')
      ...
      if (challengeToken) {
        const challengeRes = await fetch(`/api/diagnostic-challenges/${challengeToken}/submit`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            category: `ap-us-gov-diagnostic-${diagnosticResults.form}`,
  ...
  if (phase === 'testing' && testData) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 py-6 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          ...
          <div className="flex items-center justify-between">
            ...
            {currentIndex < testData.questions.length - 1 ? (
              ...
              <button onClick={handleFinish} className="rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:shadow-lg">Submit Diagnostic</button>
            )}
          </div>
        </div></div>
      </div>
  ...
  if (phase === 'results' && results) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 py-8 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          <h2 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">AP US Government Diagnostic Results</h2>
          ...
          <DiagnosticChallengeCard
            category={`ap-us-gov-diagnostic-${results.form ?? 1}`}
          ...
          {results.recommendedTopics.length > 0 && !planAccess.loading && !planAccess.canAccess && <DiagnosticPlanPaywall />}
            ...
            <div className="mb-8 rounded-2xl border-2 border-blue-300 bg-blue-50 p-6 dark:border-blue-700 dark:bg-blue-900/20">
              ...
              <p className="mb-4 text-sm text-blue-600 dark:text-blue-400">Based on your results, we recommend reviewing these {results.recommendedTopics.length} module{results.recommendedTopics.length > 1 ? 's' : ''}.</p>
              <div className="space-y-2">
            ...
            <StartStudyPlanButton diagnosticPath="/ap-us-gov-diagnostic" className="mb-8" />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-8 sm:py-12"><div className="mx-auto max-w-2xl">
        ...
        <h1 className="mb-2 text-3xl font-bold text-gray-900 sm:text-4xl dark:text-white">🏛️ AP US Government Diagnostic Test</h1>
        <p className="mb-8 text-lg text-gray-600 dark:text-gray-400">Find your strengths and weaknesses across all AP US Government units and get a personalized study plan.</p>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <ul className="mb-6 space-y-2 text-sm text-gray-600 dark:text-gray-400">
            {['~15 questions spanning all 5 AP US Government units', '35 minute time limit', 'Estimated AP score (1–5) with per-unit breakdown', '3-5 personalized module recommendations', '10 unique forms — each test has different questions'].map(item => (
              <li key={item} className="flex items-start gap-2">
                <svg className="mt-0.5 h-4 w-4 shrink-0 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                {item}
              </li>
            ))}
          </ul>
          <button onClick={startTest} className="w-full rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3 font-semibold text-white shadow-lg transition hover:shadow-xl">{lastResult ? 'Take Next Diagnostic' : 'Start Diagnostic Test'}</button>
          <Link href="/ap-us-gov-score-predictor" className="mt-3 block text-center text-sm font-medium text-blue-700 transition hover:underline dark:text-blue-300">Prefer a quick estimate? Open the Score Predictor</Link>
          <Link href="/ap-us-gov-daily-question" className="mt-1 block text-center text-sm font-medium text-blue-700 transition hover:underline dark:text-blue-300">Need a warm-up first? Try today&apos;s question</Link>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="space-y-3">
            {[
              { step: '1', text: 'Take the diagnostic test — questions cover all 5 AP US Government units' },
