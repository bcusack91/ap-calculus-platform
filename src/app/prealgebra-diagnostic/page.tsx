...
import {
  generatePreAlgebraDiagnosticTest,
  scorePreAlgebraDiagnostic,
  pickNextForm,
  TOTAL_FORMS,
  type PreAlgebraTestData,
  type PreAlgebraResults,
} from '@/data/prealgebra-diagnostic'
...
export default function PreAlgebraDiagnosticPage() {
  ...
  const [testData, setTestData] = useState<PreAlgebraTestData | null>(null)
  const [results, setResults] = useState<PreAlgebraResults | null>(null)
  ...
  useEffect(() => {
    if (status === 'authenticated') {
      fetch('/api/prealgebra-diagnostic/history')
  ...
  const startTest = useCallback(() => {
    ...
    const data = generatePreAlgebraDiagnosticTest(form)
  ...
  const handleFinish = useCallback(async () => {
    ...
    const diagnosticResults = scorePreAlgebraDiagnostic(testData.form, testData.questions, answersRecord)
    ...
    try {
      ...
      if (challengeToken) {
        const challengeRes = await fetch(`/api/diagnostic-challenges/${challengeToken}/submit`, {
          ...
          body: JSON.stringify({
            category: `prealgebra-diagnostic-${diagnosticResults.form}`,
  ...
  if (phase === 'results' && results) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-lime-50 via-white to-green-50 py-8 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div className="container"><div className="mx-auto max-w-3xl">
          <h2 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Pre-Algebra Diagnostic Results</h2>
          <div className="mb-8 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800"><p className="text-sm text-gray-500 dark:text-gray-400">Level</p><p className="text-3xl font-black text-lime-600 dark:text-lime-400">{results.estimatedLevel}</p></div>
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800"><p className="text-sm text-gray-500 dark:text-gray-400">Correct</p><p className="text-4xl font-black text-green-600 dark:text-green-400">{results.totalCorrect}/{results.totalQuestions}</p><p className="text-xs text-gray-500 dark:text-gray-400">{results.percentage}%</p></div>
          ...
          <div className="mb-8 grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl border border-lime-200 bg-lime-50 p-5 dark:border-lime-800 dark:bg-lime-900/20"><h4 className="mb-2 font-semibold text-lime-800 dark:text-lime-300">💪 Strengths</h4><ul className="space-y-1 text-sm text-lime-700 dark:text-lime-400">{results.strengths.length > 0 ? results.strengths.map(s => <li key={s}>• {s}</li>) : <li>Complete more questions to identify strengths</li>}</ul></div>
          ...
          <DiagnosticChallengeCard
            category={`prealgebra-diagnostic-${results.form ?? 1}`}
          ...
          {results.recommendedTopics.length > 0 && !planAccess.loading && !planAccess.canAccess && <DiagnosticPlanPaywall />}
            ...
            <StartStudyPlanButton diagnosticPath="/prealgebra-diagnostic" className="mb-8" />
          <div className="mb-8 rounded-2xl border border-lime-200 bg-lime-50 p-5 dark:border-lime-800 dark:bg-lime-900/20">
            <h4 className="mb-2 font-semibold text-lime-800 dark:text-lime-300">🔄 How This Cycle Works</h4>
            <ol className="space-y-2 text-sm text-lime-700 dark:text-lime-400 list-decimal list-inside">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-lime-50 via-white to-green-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
      <div className="container py-8 sm:py-12"><div className="mx-auto max-w-2xl">
        <Link href="/courses/pre-algebra" className="mb-4 inline-flex items-center gap-1 text-sm text-lime-600 hover:underline dark:text-lime-400">
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>Pre-Algebra
        </Link>
        <h1 className="mb-2 text-3xl font-bold text-gray-900 sm:text-4xl dark:text-white">📐 Pre-Algebra Diagnostic Test</h1>
        <p className="mb-8 text-lg text-gray-600 dark:text-gray-400">Identify your strengths and weaknesses across all Pre-Algebra domains and get a personalized study plan.</p>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <ul className="mb-6 space-y-2 text-sm text-gray-600 dark:text-gray-400">
            {['~30 questions spanning 6 Pre-Algebra domains', '30 minute time limit', 'Proficiency level assessment with per-domain breakdown', 'Personalized topic recommendations', `${TOTAL_FORMS} unique forms with different questions each time`].map(item => (
              <li key={item} className="flex items-start gap-2"><svg className="mt-0.5 h-4 w-4 shrink-0 text-lime-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>{item}</li>
            ))}
          </ul>
        ...
        <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="space-y-3">
            {[
              { step: '1', text: 'Take the diagnostic test — questions cover all 6 Pre-Algebra domains' },
