...
/**
 * Personalized header for the SAT course page: SAT flashcards due right now,
 * the study modules recommended by the student's last diagnostic with
 * done/to-do status, and — once every module is done — the next-diagnostic
 * call to action. Signed-out visitors see nothing (the hero handles them).
 */
export default function SATStudyPlan() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/flashcards/review?courseSlug=sat-prep&tzOffset=${new Date().getTimezoneOffset()}`)
      ...
      <div className="mx-auto max-w-5xl space-y-4">
        ...
        {cards && cards.due > 0 && (
          <Link
            ...
            className="flex items-center justify-between gap-4 rounded-2xl border-2 border-purple-300 bg-white p-5 shadow-sm transition hover:border-purple-400 hover:shadow-md dark:border-purple-700 dark:bg-gray-800 dark:hover:border-purple-500"
          >
            <div className="flex items-center gap-3">
              <span className="text-3xl">🃏</span>
              <div>
                <div className="text-lg font-bold text-gray-900 dark:text-white">
                  {cards.due} SAT flashcard{cards.due === 1 ? '' : 's'} due now
                </div>
            ...
            <span className="shrink-0 rounded-lg bg-purple-600 px-4 py-2 text-sm font-semibold text-white">
              Review now →
            </span>
          </Link>
        )}
        ...
        {hasPlan && (
          <div className="rounded-2xl border-2 border-purple-300 bg-white p-6 shadow-sm dark:border-purple-700 dark:bg-gray-800">
                  ...
                  <Link
                    ...
                    className={`flex items-center justify-between gap-3 rounded-xl border px-4 py-3 transition ${
                      topic.isSatisfied
                        ...
                        : 'border-gray-200 hover:border-purple-300 hover:bg-purple-50/50 dark:border-gray-600 dark:hover:border-purple-500'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                    ...
                    <span className={`shrink-0 text-sm font-semibold ${topic.isSatisfied ? 'text-green-700 dark:text-green-400' : 'text-purple-700 dark:text-purple-400'}`}>
