...
;(async () => {
  ...
  const cols = await p.$queryRaw<{ table_name: string; column_name: string }[]>`
    ...
    WHERE (table_name = 'CompetitiveLobby' AND column_name IN ('isPublic','topicSlug','gameMode','difficulty','hostLastSeenAt'))
       OR (table_name = 'TeacherLobby' AND column_name IN ('studentHosted','isPublic','maxPlayers','format','difficulty','hostLastSeenAt'))
    ORDER BY 1, 2`
  console.log(`\nverified ${cols.length}/11 columns present`)
  process.exit(0)
})()
