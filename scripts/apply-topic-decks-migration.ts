/**
 * Applies the topic-slide-decks migration.
 *   Local:  npx tsx scripts/apply-topic-slide-decks-migration.ts
 *   Prod:   PROD=1 npx tsx scripts/apply-topic-slide-decks-migration.ts
...
async function main() {
  ...
  const sql = readFileSync('prisma/migrations/20260813100000_add_topic_slide_decks/migration.sql', 'utf8')
  ...
  const tables = await prisma.$queryRawUnsafe<{ table_name: string }[]>(
    `SELECT table_name FROM information_schema.tables WHERE table_name IN ('TopicSlideDeck')`)
