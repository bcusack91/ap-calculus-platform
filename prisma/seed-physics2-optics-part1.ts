...
async function main() {
  console.log('Seeding AP Physics 2 - Optics Part 1...')
  ...
  console.log('✓ Created category: Optics')
  ...
  await prisma.flashcard.createMany({
    data: [
      ...
      },
      {
        ...
        back: 'n₁sinθ₁ = n₂sinθ₂. Angles from normal. Fast→Slow (n₁<n₂): bends toward normal. Slow→Fast (n₁>n₂): bends away from normal. Frequency constant, v and λ change.',
      },
      {
        topicId: reflectionRefraction.id,
        front: 'What is the index of refraction? Give formula and typical values.',
        back: 'n = c/v, where v is speed of light in material. n ≥ 1, dimensionless. Vacuum: n=1, air≈1, water=1.33, glass≈1.5, diamond=2.42. Higher n → slower light.',
      },
      {
        topicId: reflectionRefraction.id,
      ...
      {
        topicId: reflectionRefraction.id,
        front: 'What causes dispersion? Why does a prism create a spectrum?',
        back: 'Different wavelengths have different n in same material. Violet: higher n, bends more. Red: lower n, bends less. White light separates into ROYGBIV spectrum.',
      },
      {
        topicId: reflectionRefraction.id,
      ...
      {
        topicId: reflectionRefraction.id,
        front: 'What happens to frequency, wavelength, and speed when light refracts?',
        back: 'Frequency: CONSTANT (determined by source). Speed: v = c/n (changes). Wavelength: λ = v/f (changes). Higher n → lower v and λ, but same f.',
      },
    ],
  })

  console.log('✓ Created 10 flashcards for Reflection and Refraction')

  console.log('\n✅ Successfully seeded AP Physics 2 - Optics Part 1!')
