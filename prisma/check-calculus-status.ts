...
async function main() {
  console.log('Checking AP Calculus AB/BC course status...\n');

  // Find the AP Calculus course
  const calculusCourse = await prisma.course.findFirst({
    where: { slug: 'ap-calculus-ab-bc' },
    include: {
      categories: {
        include: {
          topics: {
  ...
  if (!calculusCourse) {
    console.log('❌ AP Calculus AB/BC course not found!');
    await prisma.$disconnect();
    return;
  }

  console.log(`✅ Found course: ${calculusCourse.name}`);
  console.log(`Categories: ${calculusCourse.categories.length}\n`);

  let totalTopics = 0;
  let topicsWithEnoughProblems = 0;
  let topicsWithEnoughFlashcards = 0;

  calculusCourse.categories.forEach((category) => {
    console.log(`\n📁 ${category.name}:`);
